...
Sử dụng:
    from midicoder.emitters.core.cp01_domain_model import EntityParser
...
class EntityParser:
    ...
    def parse(self, yaml_content: str) -> list[Entity]:
        ...
        if not data or "entities" not in data:
            EM.raise_error(
                ErrorCode.CP01_ENTITY_NOT_FOUND,
    ...
    def _parse_field(self, field_def: dict[str, Any]) -> Field:
        ...
        if not field_def.get("name"):
            EM.raise_error(
                ErrorCode.CP01_ENTITY_INVALID_FIELD,
    ...
    def _parse_relationship(
        ...
        if not target:
            EM.raise_error(
                ErrorCode.CP01_ENTITY_INVALID_RELATIONSHIP,
    ...
    def _parse_constraint_type(self, type_str: str) -> ConstraintType:
        ...
        if type_lower not in type_mapping:
            EM.raise_error(
                ErrorCode.CP01_ENTITY_INVALID_CONSTRAINT,
    ...
    def _parse_lifecycle_hooks(
        ...
        for event_str, hooks_list in lifecycle_def.items():
            ...
            if not event:
                EM.raise_error(
                    ErrorCode.CP01_ENTITY_INVALID_LIFECYCLE,
