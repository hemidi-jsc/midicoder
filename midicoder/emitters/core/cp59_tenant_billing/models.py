    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            raise EM.raise_error(
                ErrorCode.CP59_BILLING_PLAN_NOT_FOUND,
                reason="plan_id bắt buộc và không được để trống",
            )

        if self.monthly_price < 0:
            raise EM.raise_error(
                ErrorCode.CP59_INVALID_BILLING_AMOUNT,
...
class BillingCycle:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            raise EM.raise_error(
                ErrorCode.CP59_BILLING_CYCLE_NOT_FOUND,
                reason="cycle_id bắt buộc và không được để trống",
            )

        if self.end_date <= self.start_date:
            raise EM.raise_error(
                ErrorCode.CP59_INVALID_BILLING_CYCLE,
...
class InvoiceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            raise EM.raise_error(
                ErrorCode.CP59_INVOICE_NOT_FOUND,
...
class UsageMeter:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            raise EM.raise_error(
                ErrorCode.CP59_USAGE_METER_NOT_FOUND,
...
class PaymentGatewayConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            raise EM.raise_error(
                ErrorCode.CP59_PAYMENT_GATEWAY_NOT_FOUND,
