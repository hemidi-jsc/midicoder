...
class VersioningParser:
    ...
    def parse(self, raw: str) -> VersioningCollection:
        ...
        try:
            ...
            EM.raise_error(ErrorCode.CP43_DSL_PARSE_ERROR, error=str(e))

        # YAML comment-only hoặc null → treat as empty collection
        if data is None:
            return VersioningCollection()
        if not isinstance(data, dict):
            EM.raise_error(ErrorCode.CP43_DSL_PARSE_ERROR, reason="DSL versioning phải là YAML mapping")
    ...
    def _parse_version_config(self, data: dict[str, Any]) -> VersionConfig:
        ...
        """
        if not isinstance(data, dict):
            EM.raise_error(ErrorCode.CP43_DSL_PARSE_ERROR, reason="Versioning config phải là YAML mapping")

        entity_type = data.get("entity_type", "")
        if not entity_type:
            EM.raise_error(ErrorCode.CP43_INVALID_VERSION_CONFIG, field="entity_type")
    ...
    def _parse_history_record(self, data: dict[str, Any]) -> HistoryRecord:
        ...
        """
        if not isinstance(data, dict):
            EM.raise_error(ErrorCode.CP43_DSL_PARSE_ERROR, reason="History record phải là YAML mapping")
        ...
        try:
            ...
            EM.raise_error(
                ErrorCode.CP43_INVALID_VERSION_CONFIG,
