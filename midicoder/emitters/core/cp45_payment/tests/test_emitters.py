...
from midicoder.emitters.core.cp45_payment.parser import parse_to_ir, PaymentIR
...
class TestFastAPIEmitter:
    @pytest.fixture
    def emitter(self):
        from midicoder.emitters.core.cp45_payment.fastapi import FastAPIPaymentEmitter
...
class TestNestJSEmitter:
    @pytest.fixture
    def emitter(self):
        from midicoder.emitters.core.cp45_payment.nestjs import NestJSPaymentEmitter
        stacks_dir = PACKAGE_DIR / "stacks"
...
class TestAngularEmitter:
    @pytest.fixture
    def emitter(self):
        from midicoder.emitters.core.cp45_payment.angular import AngularPaymentEmitter
        stacks_dir = PACKAGE_DIR / "stacks"
...
class TestReactEmitter:
    @pytest.fixture
    def emitter(self):
        from midicoder.emitters.core.cp45_payment.react import ReactPaymentEmitter
        stacks_dir = PACKAGE_DIR / "stacks"
...
class TestPackManifest:
    ...
    def test_pack_internal_id(self):
        with open(PACK_DIR / "pack.yml", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        assert data["pack"]["internal_id"] == "cp45_payment"
...
class TestStackTemplates:
    ...
    def _check_templates(self, stack_dir_name, expected_count):
        stacks_dir = PACKAGE_DIR / "stacks"
        template_dir = stacks_dir / stack_dir_name / "core" / "cp45_payment"
...
class TestRegistry:
    def test_cp45_in_registry(self):
        ...
        assert CP_ID_TO_INTERNAL["CP45"] == "cp45_payment"
...
class TestInitModule:
    ...
    def test_init_exports_models(self):
        from midicoder.emitters.core.cp45_payment import (
            PaymentEngine,
            PaymentTransaction,
            PaymentMethod,
            PaymentRefund,
