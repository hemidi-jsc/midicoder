...
class PaymentGatewayConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.timeout_seconds < 1:
            raise EM.raise_error(
                ErrorCode.CP45_INVALID_PAYMENT_AMOUNT,
...
class PaymentRefund:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.refund_id or not self.refund_id.strip():
            raise EM.raise_error(
                ErrorCode.CP45_REFUND_NOT_ALLOWED,
...
class PaymentEngine:
    ...
    def set_default_payment_method(self, user_id: str, method_id: str) -> PaymentMethod:
        ...
        if not method.is_usable:
            raise EM.raise_error(
                ErrorCode.CP45_PAYMENT_METHOD_EXPIRED,
    ...
    def process_payment(
        ...
        if idempotency_key and idempotency_key in self.idempotency_keys:
            ...
            raise EM.raise_error(
                ErrorCode.CP45_PAYMENT_IDEMPOTENCY_CONFLICT,
    ...
    def create_refund(
        ...
        if refund_amount > tx.amount:
            raise EM.raise_error(
                ErrorCode.CP45_REFUND_AMOUNT_EXCEEDS_ORIGINAL,
    ...
    def verify_webhook_signature(
        ...
        if not signature:
            raise EM.raise_error(
                ErrorCode.CP45_WEBHOOK_SIGNATURE_INVALID,
