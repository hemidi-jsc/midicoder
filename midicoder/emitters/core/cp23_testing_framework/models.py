...
class TestAssertion:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.check:
            EM.raise_error(ErrorCode.CP23_INVALID_ASSERTION, field="check")
...
class TestCase:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.CP23_EMPTY_TEST_ID, field="test_case.id")
        if not self.target or not self.target.strip():
            EM.raise_error(ErrorCode.CP23_MISSING_TEST_TARGET, test_id=self.id)
        # E2E tests must have steps
        if self.type == TestType.E2E and not self.steps:
            EM.raise_error(
                ErrorCode.CP23_MISSING_TEST_TARGET,
                test_id=self.id,
...
class TestPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.coverage_threshold <= 100:
            EM.raise_error(
                ErrorCode.CP23_INVALID_COVERAGE_THRESHOLD,
...
class TestSuite:
    ...
    def add_case(self, case: TestCase) -> None:
        ...
        if self.get_case_by_id(case.id):
            EM.raise_error(ErrorCode.CP23_DUPLICATE_TEST_ID, id=case.id)
