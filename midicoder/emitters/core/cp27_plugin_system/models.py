...
class PluginSlot:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.CP27_EMPTY_SLOT_ID, field="slot.id")
...
class PluginContract:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.CP27_PLUGIN_CONTRACT_VIOLATION, field="contract.id", reason="Contract ID không được để trống")
...
class PluginPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.CP27_EMPTY_POLICY_ID, field="policy.id")
        if not self.policy_type:
            EM.raise_error(ErrorCode.CP27_INVALID_POLICY_TYPE, field="policy.policy_type")
...
class PluginManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.CP27_PLUGIN_NOT_FOUND, field="manifest.id", reason="Manifest ID không được để trống")
...
class PluginCollection:
    ...
    def add_contract(self, contract: PluginContract) -> None:
        ...
        if self.get_contract_by_id(contract.id):
            EM.raise_error(ErrorCode.CP27_PLUGIN_CONTRACT_VIOLATION, id=contract.id, reason="Duplicate contract ID")
        self.contracts.append(contract)

    def add_policy(self, policy: PluginPolicy) -> None:
        """Thêm plugin policy vào collection."""
        if self.get_policy_by_id(policy.id):
            EM.raise_error(ErrorCode.CP27_PLUGIN_POLICY_VIOLATION, id=policy.id, reason="Duplicate policy ID")
