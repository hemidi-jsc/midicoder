"""
Tests cho CP65 NestJS Backup Emitter.

Unit tests cho:
- NestJSBackupEmitter init (requires valid stack_dir)
...
from midicoder.emitters.core.cp65_backup_recovery.models import (
    BackupPolicy,
    BackupType,
    RecoveryPlan,
    RecoveryStep,
...
from midicoder.emitters.core.cp65_backup_recovery.nestjs import (
    NestJSBackupEmitter,
    GeneratedFile,
)
...
# ===========================================================================
# Test NestJSBackupEmitter
# ===========================================================================


class TestNestJSBackupEmitter:
    """Tests cho NestJSBackupEmitter."""
    ...
    def stack_dir(self) -> Path:
        ...
        # Tạo mock templates
        for name in [
            "backup.module.ts.jinja2",
            "backup.service.ts.jinja2",
            "restore.service.ts.jinja2",
            "backup.controller.ts.jinja2",
        ]:
            (template_dir / name).write_text("{{ policy_count }} files from cp65 nestjs")
        return tmp
    ...
    def test_emit_file_paths(self, backup_ir: BackupIR, stack_dir: Path, output_dir: Path):
        ...
        assert "src/backup/backup.module.ts" in paths
        assert "src/backup/backup.service.ts" in paths
        assert "src/backup/restore.service.ts" in paths
        assert "src/backup/backup.controller.ts" in paths
