...
def _prepare_contract_generation(paths: MidicoderPaths, version: str, root: Path) -> dict[str, Any]:
...
def gen(root: Path) -> None:
    ...
    print("[contract gen] Analyzing master brief to determine required contract files...")
    ...
    try:
        ...
        print(f"[contract gen] LLM determined {len(required_files)} required contract files")
        ...
        write_run_outputs(run_dir, "contract_gen", summary, state_before=state, state_after=state)
...
def _generate_contract_files(
    *,
    ...
    print(f"[{task_name}] Starting contract generation: {len(required_files)} file(s) in {total_passes} pass(es)...")
    ...
    try:
        ...
        for index, batch in enumerate(batches, start=1):
            target_file = batch[0]
            print(f"[{task_name}] Pass {index}/{total_passes}: generating {target_file}")
            ...
            try:
                ...
                print(f"[{task_name}] Pass {index}/{total_passes} completed: wrote {pass_created} new file(s)")
                
            except Exception as pass_exc:
                # Save error artifacts and build enhanced error message
                enhanced_error = save_error_artifacts(
                    run_dir, index, pass_exc, target_file, response, response_text, task_name
                )
    ...
    summary = build_generation_summary(
        version, status, created_files, llm_config, stack_target,
        task_name, required_files, all_traces, error_message
    )

    write_run_outputs(run_dir, task_name, summary, state_before=state, state_after=state)
    
    if status == "generated":
        print(f"[{task_name}] Generated {len(created_files)} file(s) for version {version}")
    else:
        from midicoder.llm.client import LlmRequestError
...
def check(root: Path) -> None:
    ...
    if master_brief.exists():
        try:
            ...
            print("[contract check] Warning: Could not analyze master brief, checking all existing contract files")
            expected_files = list(contracts_root.glob("**/*.yaml")) if contracts_root.exists() else []
    else:
        expected_files = list(contracts_root.glob("**/*.yaml")) if contracts_root.exists() else []
    ...
    issue_payload = [{"location": issue.location, "message": issue.message} for issue in issues]
    ...
    print(f"[contract check] FAILED – {len(issues)} issue(s) found in {len(checked_files)} file(s)")
...
def feedback(root: Path) -> None:
    ...
    if new_items_count:
        print(f"[contract feedback] {feedback_path} ({items_count} items, {new_items_count} new from contract check)")
    else:
...
def repair_prepare(root: Path) -> None:
    ...
    if not feedback_path.exists():
        raise RuntimeError("contract-feedbacks.yml is missing. Run `contract feedback` first.")

    feedback, errors = validate_feedback(paths, version=version, feedback_path=feedback_path)
...
def repair_run(root: Path) -> None:
    ...
    feedback, errors = validate_feedback(paths, version=str(version), feedback_path=feedback_path)
    if errors:
    ...
    print(f"[contract repair run] Processing {len(pending_items)} pending feedback items...")
    ...
    print(f"[contract repair run] Grouped into {len(grouped_files)} files for file-level repair")
    ...
    for i, file_path in enumerate(ordered_files, 1):
        ...
        print(f"[contract repair run] Processing file {i}/{len(ordered_files)}: {file_path} ({len(file_items)} items)")
    ...
    print(f"[contract repair run] Completed: {successful_files}/{len(ordered_files)} files successful")
    
    if failed_files:
        print(f"[contract repair run] Failed files ({len(failed_files)}): {', '.join(failed_files)}")
...
def gen_resume(root: Path) -> None:
    ...
    if not contract_gen_runs_dir.exists():
        print("[contract gen resume] No previous contract_gen runs found. Use `contract gen` instead.")
        return
    ...
    try:
        ...
        print(f"[contract gen resume] Expected contract files: {len(required_files)} files")
    except Exception as exc:
    ...
    if not remaining_files:
        print("[contract gen resume] All contract files are already generated and valid.")
        return
    
    print_resume_summary(completed_files, failed_files, set(), remaining_files, "contract gen resume")
