...
def test(root: Path, args: argparse.Namespace) -> None:
    ...
    from midicoder.io.messages import print_error, print_info, print_normal, print_success
    ...
    if result.success:
        ...
        print_normal("  1. Review error details: [cyan]cat " + result.log_dir + "/error.log[/cyan]")
...
def fix(root: Path, args: argparse.Namespace) -> None:
    ...
    from midicoder.io.messages import print_error, print_info, print_normal, print_success, print_warning
    ...
    if args.dry_run and (auto_apply or auto_fix_loop):
        print_warning("Ignoring --auto-apply/--auto-fix-loop because --dry-run is enabled.")
        auto_apply = False
        auto_fix_loop = False
...
def _print_fix_result(result, dry_run: bool) -> None:
    ...
    if result.success:
        ...
        if dry_run:
            ...
            print_normal("  1. Review patches: [cyan]ls " + result.patches_dir + "[/cyan]")
