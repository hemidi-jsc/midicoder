...
def get_line_range(
    ...
    try:
        ...
        if hasattr(target_obj, "lc"):
            ...
            if isinstance(target_obj, dict) and hasattr(target_obj.lc, "data"):
                ...
                for k in target_obj.keys():
                    key_line = (
                        target_obj.lc.key(k) if hasattr(target_obj.lc, "key") else None
                    )
                    if key_line:
                        all_lines.append(key_line[0])
                    value_line = (
                        target_obj.lc.value(k)
                        if hasattr(target_obj.lc, "value")
                        else None
                    )
                    if value_line:
        ...
        if hasattr(target_obj, "__line_start__") and hasattr(
            target_obj, "__line_end__"
        ):
...
def format_line_context(
    file_path: str, line_number: int, context_lines: int = 2
) -> str:
    """
...
def extract_line_info(obj: Any, key: str | int | None = None) -> dict[str, int | None]:
    ...
    return {
        ...
        "end": line_end,
        "single": line_single
        or line_start,  # Fallback to start if single not available
    }
