...
def apply_intent_module(
    ...
    route_candidates = _collect_route_module_candidates(validated_data, normalizer, schema_tree)
    ...
    workflow_candidates = _collect_workflow_command_candidates(validated_data, normalizer, ir)
    workflow_event_candidates = _collect_workflow_event_candidates(validated_data, normalizer, ir)
...
def validate_intents(
    ...
    for _, elements in target_groups:
        for element in elements:
            if element.intent is None or not element.intent.kind or not element.intent.module:
                reporter.add_error(
                    stage="intent",
                    code=E321,
...
def _collect_route_module_candidates(
    ...
    for _, data in validated_data.items():
        ...
        for route in data.routes:
            ...
            route_command_ids, route_query_ids = _resolve_http_route_targets(route, normalizer)
...
def _resolve_http_route_targets(route: Any, normalizer: Normalizer) -> tuple[set[str], set[str]]:
    ...
    if getattr(route, "command", None):
        ...
        _assign_http_target(
            command_id, normalizer, preferred_type="command", command_ids=command_ids, query_ids=query_ids
        )

    if getattr(route, "query", None):
        query_id = _normalize_ref_id(route.query, normalizer)
        _assign_http_target(
            query_id, normalizer, preferred_type="query", command_ids=command_ids, query_ids=query_ids
        )
...
def _infer_module_for_element(
    ...
    file_module = _module_from_file_path(source_file, allow_file_stem=allow_file_stem, schema_tree=schema_tree)
    if file_module:
        _consider_candidate(
            resolved,
            ModuleCandidate(
...
def _module_from_http_path(path: str, schema_tree: dict[str, Any] | None = None) -> str | None:
...
def _module_from_file_path(path: str, allow_file_stem: bool, schema_tree: dict[str, Any] | None = None) -> str | None:
    ...
    if schema_tree:
        ...
        ignore_dirs = {"app", "domain", "api", "workflows", "policy", "rules", "scenarios"}
...
def _module_from_id_pattern(element_id: str, schema_tree: dict[str, Any] | None = None) -> str | None:
...
def _pluralize_module_token(token: str, schema_tree: dict[str, Any] | None = None) -> str:
