...
class CommandGraphGenerator(DiagramGenerator):
    ...
    def generate(self, application: ApplicationIR, domain: DomainIR) -> list[DiagramOutput]:
    ...
    def _build_mermaid_source(self, application: ApplicationIR, domain: DomainIR) -> str:
        lines = [
            "---",
        ...
        if guard_defs:
            ...
            for guard_id in guard_defs:
                ...
                lines.append(f"        {node_id}((\"{MermaidRenderer.escape_mermaid_text(guard_id)}\"))")
            lines.append("    end")
            lines.append("")
        ...
        if effect_defs:
            ...
            for effect_id in effect_defs:
                ...
                lines.append(f"        {node_id}((\"{MermaidRenderer.escape_mermaid_text(effect_id)}\"))")
            lines.append("    end")
            lines.append("")
        ...
        for command in application.commands:
            ...
            for fetch_ref in command.fetches:
                if fetch_ref.type == "Entity" and fetch_ref.id in entity_map:
                    ent_node = MermaidRenderer.sanitize_mermaid_id(f"ent_{fetch_ref.id}")
