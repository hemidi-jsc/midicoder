...
class WorkflowDiagramGenerator(DiagramGenerator):
    ...
    def _is_terminal_state(self, state: StateIR) -> bool:
        ...
        """
        if "final" in state.id.lower() or "end" in state.id.lower() or "terminal" in state.id.lower():
            return True
    ...
    def _build_transition_label_mermaid(self, transition: TransitionIR) -> str:
        ...
        if transition.guards:
            guard_ids = [MermaidRenderer.sanitize_mermaid_id(g.id) for g in transition.guards]
            if len(guard_ids) <= 2:
        ...
        if transition.effects:
            effect_ids = [MermaidRenderer.sanitize_mermaid_id(e.id) for e in transition.effects]
            if len(effect_ids) <= 2:
