...
class PolicyDiagramGenerator(DiagramGenerator):
    ...
    def _generate_access_diagram(self, access: AccessPolicyIR) -> DiagramOutput | None:
        ...
        for role in access.roles:
            ...
            lines.append(f"        +Role {MermaidRenderer.escape_mermaid_text(role.id)}")
            if role.description:
                lines.append(f"        {MermaidRenderer.escape_mermaid_text(role.description[:60])}")
            lines.append("    }")
        ...
        for perm in access.permissions:
            ...
            lines.append(f"    class {node_id}{{")
            lines.append(f"        +Perm {MermaidRenderer.escape_mermaid_text(perm.id)}")
            lines.append(f"        resource: {MermaidRenderer.escape_mermaid_text(perm.resource)}")
            if actions:
                lines.append(f"        actions: {MermaidRenderer.escape_mermaid_text(actions)}")
            lines.append("    }")
        ...
        for perm in access.permissions:
            ...
            if res not in resource_nodes:
                ...
                lines.append(f"    class {node_id}{{")
                lines.append(f"        +Resource {MermaidRenderer.escape_mermaid_text(res)}")
                lines.append("    }")
            lines.append(f"    {permission_nodes[perm.id]} --> {resource_nodes[res]} : grants")
        ...
        for binding in access.bindings:
            ...
            for perm_id in binding.permissions:
                ...
                scope_label = f" : {MermaidRenderer.escape_mermaid_text(scope)}" if scope else ""
        ...
        sources = []
        for role in access.roles:
            sources.append(create_source_metadata(
                id=role.id,
                type="Role",
    ...
    def _generate_business_diagram(self, policies: list[BusinessPolicyIR]) -> DiagramOutput | None:
        if not policies:
            return None
        ...
        for policy in policies[:20]:
            ...
            for idx, condition in enumerate(policy.conditions[:4]):
                cond_node = MermaidRenderer.sanitize_mermaid_id(f"{policy.id}_cond_{idx}")
            ...
            for idx, effect in enumerate(policy.effects[:4]):
                effect_node = MermaidRenderer.sanitize_mermaid_id(f"{policy.id}_effect_{idx}")
