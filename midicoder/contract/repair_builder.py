    ...
    *,
    ...
    for i, item in enumerate(feedback_items, 1):
        ...
        if not target_item_id:
            target_item_id = fallback_target_item_id(item.location, item.file) or "unknown"
...
- **Format**: {location_format}""")
    ...
    if "symbols" in context:
        if isinstance(context["symbols"], dict):
            symbols_count = len(context["symbols"].get("entities", [])) + len(context["symbols"].get("classes", []))
        else:
    ...
    title = f"# File-Level Contract Repair ({len(feedback_items)} Items)" if is_multi_item else "# Contract Repair Task"
    mission = f"Execute 3-step repair for **{len(feedback_items)} items** in `{file_path}`" if is_multi_item else f"Execute 3-step repair for `{first_item.id}` in `{file_path}`"
    ...
    return "\n".join([
        ...
        "",
        "**Existing IDs:**",
        "\n".join([f"- **{k}**: {', '.join(v)}" for k, v in existing_ids.items()]) if existing_ids else "None.",
        "",
        "**Available Reference IDs:**",
        "\n".join([f"- **{k}**: {', '.join(v)}" for k, v in available_refs.items()]) if available_refs else "None needed.",
        "",
        ...
        "```yaml",
        "patches:",
    ] + [f"  - file: \"{file_path}\"\n    kind: \"replace_node\"\n    location: \"{loc}\"\n    new_content: |\n      # Complete YAML content here" for loc in patch_locations] + [
        "",
        "feedback_status_updates:",
    ] + [f"  - id: \"{item.id}\"\n    status: \"done\"\n    last_error: null" for item in feedback_items] + [
        "",
        "memos:",
...
def get_patch_location_template(location: str, target_item_id: str | None = None) -> str:
    ...
    for i, part in enumerate(parts):
        if part.isdigit():
            ...
            elif target_item_id and any(target_item_id.startswith(p) for p in ["GET_", "POST_", "PUT_", "DELETE_", "item_", "route_"]):
                return f"{path_to_array}.{part}"
            else:
...
def extract_existing_ids_from_file(contract_path: Path) -> dict[str, list[str]]:
    ...
    from midicoder.contract.contract_utils import load_yaml_safe, extract_ids_from_yaml_list
    ...
    try:
        ...
        existing_ids = {}
        common_lists = ["commands", "queries", "entities", "errors", "events", "routes", "terms"]
...
def save_repair_debug_files(
    ...
    (debug_dir / "validation_errors.json").write_text(
        json.dumps(current_validation_errors, indent=2, ensure_ascii=False), encoding="utf-8"
    )
...
def extract_schema_cheatsheet_for_debugging(context: dict, file_path: str) -> dict:
    ...
    if not schema_cheatsheet:
        ...
        file_without_contracts = file_path.replace("contracts/", "") if file_path.startswith("contracts/") else file_path
        sliced_schema = slice_schema_for_targets(context.get("schema_tree", {}), [file_without_contracts])
        schema_cheatsheet = {
...
def get_file_validation_errors(contract_path: Path, file_path: str) -> list[dict]:
    ...
    try:
        ...
        return [{"location": str(contract_path), "message": f"Validation failed: {exc}"}]
