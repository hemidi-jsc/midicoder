    ...
    *,
    ...
    for patch in patches:
        ...
        if kind not in SUPPORTED_PATCH_KINDS:
            errors.append(f"Unsupported patch kind: {kind}. Supported: {', '.join(SUPPORTED_PATCH_KINDS)}")
            continue
        ...
        try:
            ...
            if not target_path.exists():
                errors.append(f"Patch target not found: {file_path} (resolved to: {target_path})")
                continue
        ...
        success = False
        if kind == "replace_all":
            success = _apply_replace_all_patch(target_path, new_content, errors, file_path)
        else:
            ...
            try:
                ...
                print(f"[patch] YAML parsing failed for {file_path}, using direct text replacement: {yaml_exc}")
                if kind == "replace_node":
                    success = _apply_direct_text_replacement(target_path, str(location), new_content, errors, file_path)
                else:
                    errors.append(f"Cannot apply {kind} patch to malformed YAML: {file_path}, {yaml_exc}")
                    continue
                if success:
                    updated_files.add(target_path)
                    continue
                else:
                    errors.append(f"Failed to apply patch to malformed YAML: {file_path}, {yaml_exc}")
                    continue
            ...
            if kind == "replace_node":
                success = _apply_replace_node_patch(data, str(location), new_content, errors, file_path, yaml_loader)
            elif kind == "append":
                success = _apply_append_patch(data, str(location), new_content, errors, file_path, yaml_loader)
            elif kind == "prepend":
                success = _apply_prepend_patch(data, str(location), new_content, errors, file_path, yaml_loader)
            else:
...
def _apply_replace_all_patch(target_path: Path, new_content: Any, errors: list[str], file_path: str) -> bool:
...
def _apply_replace_node_patch(data: dict, location: str, new_content: Any, errors: list[str], file_path: str, yaml_loader: Any) -> bool:
    ...
    if _is_array_index_path(location):
        return _apply_array_index_path_patch(data, location, new_content, errors, file_path, yaml_loader)
    elif _is_yaml_path(location):
        return _apply_yaml_path_patch(data, location, new_content, errors, file_path, yaml_loader)
        ...
        if '[' in location and ']' in location:
            ...
            if _apply_array_index_path_patch(data, location, new_content, parser_errors, file_path, yaml_loader):
                return True
            errors.append(f"Invalid location format '{location}' - use proper array index path")
            return False
...
def _apply_append_patch(data: dict, location: str, new_content: Any, errors: list[str], file_path: str, yaml_loader: Any) -> bool:
    ...
    if target is None:
        ...
        target = _navigate_to_location(data, location)
        if target is None:
            errors.append(f"Failed to create location for append: {location} in {file_path}")
            return False
    
    if not isinstance(target, list):
        errors.append(f"Cannot append to non-list at location {location} in {file_path}")
        return False
...
def _apply_prepend_patch(data: dict, location: str, new_content: Any, errors: list[str], file_path: str, yaml_loader: Any) -> bool:
    ...
    if target is None:
        ...
        target = _navigate_to_location(data, location)
        if target is None:
            errors.append(f"Failed to create location for prepend: {location} in {file_path}")
            return False
    
    if not isinstance(target, list):
        errors.append(f"Cannot prepend to non-list at location {location} in {file_path}")
        return False
...
def _apply_direct_text_replacement(target_path: Path, location: str, new_content: str, errors: list[str], file_path: str) -> bool:
    ...
    try:
        ...
        for line in lines:
            ...
            if in_target_array and line and not line.startswith(' ') and not line.startswith('\t') and ':' in line:
                in_target_array = False
                skip_until_next_item = False
...
def _ensure_location_exists(data: dict, location: str, errors: list[str], file_path: str) -> bool:
    """
    ...
    array_match = re.match(r'^([a-zA-Z_][a-zA-Z0-9_]*)\[(\d+)\]\.([a-zA-Z_][a-zA-Z0-9_]*)$', location)
    if array_match:
...
def _apply_array_index_path_patch(data: dict, location: str, new_content: Any, errors: list[str], file_path: str, yaml_loader: Any) -> bool:
        ...
        for i, (name, index) in enumerate(components):
            ...
            if index is not None:
                if not isinstance(current, dict) or name not in current:
                    errors.append(f"Array '{name}' not found at path '{path_for_error}' in {file_path}")
                    return False
                
                array = current[name]
                if not isinstance(array, list):
                    errors.append(f"'{name}' is not an array at path '{path_for_error}' in {file_path}")
                    return False
                
                if index >= len(array):
                    errors.append(f"Array index {index} out of range for '{name}' at path '{path_for_error}' in {file_path}")
                    return False
                ...
                if is_final:
                    if not isinstance(current, dict):
                        errors.append(f"Cannot set field '{name}' on non-object at path '{path_for_error}' in {file_path}")
                        return False
                    ...
                    if not isinstance(current, dict):
                        errors.append(f"Cannot navigate through non-dict at '{name}' in path '{path_for_error}' in {file_path}")
                        return False
                    if name not in current:
                        ...
                        if i < len(components) - 1:
                            ...
                            errors.append(f"Field '{name}' not found at path '{path_for_error}' in {file_path}")
                            return False
                    current = current[name]
        
        errors.append(f"Unexpected end of path navigation for '{location}' in {file_path}")
        return False
        
    except Exception as exc:
        errors.append(f"Failed to apply array index path patch '{location}' in {file_path}: {exc}")
        return False


def _apply_yaml_path_patch(data: dict, location: str, new_content: Any, errors: list[str], file_path: str, yaml_loader: Any) -> bool:
    ...
    try:
        ...
        for i, part in enumerate(parts[:-1]):
            if part.isdigit():
                index = int(part)
                if not isinstance(current, list) or index >= len(current):
                    errors.append(f"Invalid array index {part} in path {location} in {file_path}")
                    return False
                current = current[index]
            else:
                if not isinstance(current, dict):
                    errors.append(f"Cannot navigate through non-dict at '{part}' in {location} in {file_path}")
                    return False
                if part not in current:
                    errors.append(f"Path component '{part}' not found in {location} in {file_path}")
                    return False
                current = current[part]
        ...
        if final_part.isdigit():
            ...
            if not isinstance(current, list) or index >= len(current):
                errors.append(f"Invalid array index {final_part} in path {location} in {file_path}")
                return False
            ...
            if not isinstance(current, dict):
                errors.append(f"Cannot set property '{final_part}' on non-object in {location} in {file_path}")
                return False
        ...
        errors.append(f"Failed to apply YAML path patch {location} in {file_path}: {exc}")
        return False
