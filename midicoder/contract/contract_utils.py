...
def analyze_run_logs(run_dir: Path, required_files: list[str]) -> tuple[set[str], set[str]]:
    """
    ...
    try:
        ...
        for response_file in response_files:
            try:
                ...
                for required_file in required_files:
                    if f"file: contracts/{required_file}" in llm_content or f"file: {required_file}" in llm_content:
                        attempted_files.add(required_file)
                        ...
                        if "finish_reason" in content:
                            try:
                                ...
                                if finish_reason == "stop":  # Complete response
                                    ...
                                    if "content:" in llm_content and not llm_content.strip().endswith("type"):
...
def issue_to_feedback_target(contracts_root: Path, issue_location: str) -> tuple[str, str]:
    ...
    try:
        relative_path = path_object.resolve().relative_to(contracts_root.resolve()).as_posix()
...
def validate_single_contract_file(contract_path: Path, file_path: str) -> list[dict[str, str]]:
    """
    ...
    try:
        ...
        for issue in all_issues:
            ...
            if file_path_normalized in issue_location or str(contract_path) in issue_location:
        ...
        try:
            from midicoder.dsl.loader import (
                load_glossary, load_entities, load_errors, load_events, 
                load_commands, load_http, load_info, load_rules,
                load_workflows, load_persistence, load_integrations,
                load_profiles, load_secrets_contract, load_security_baseline,
                load_reliability, load_observability, load_testing
            )
            ...
            for error in val_exc.errors():
                ...
                validation_errors.append({
                    "location": f"{contract_path}:{location}" if location else str(contract_path),
                    "message": message
                })
        except Exception:
            pass
...
def format_validation_errors(validation_errors: list[dict[str, str]]) -> str:
    ...
    if len(validation_errors) > MAX_VALIDATION_ERRORS_DISPLAY:
        formatted_errors.append(f"... and {len(validation_errors) - MAX_VALIDATION_ERRORS_DISPLAY} more validation errors")
