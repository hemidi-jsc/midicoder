...
def process_file_feedback_items(
    *,
    ...
    """
    from midicoder.contract.contract_utils import validate_single_contract_file, normalize_contract_path
    ...
    from midicoder.contract.yaml_processor import strip_yaml_code_fences, parse_repair_plan
    ...
    available_refs = extract_available_reference_ids(contracts_root, current_validation_errors, feedback_items)
    ...
    file_dir = run_dir / f"file_{file_index}_{file_path.replace('/', '_').replace('.yaml', '')}"
    save_repair_debug_files(
        debug_dir=file_dir,
        prompt=prompt,
        context=context,
    ...
    try:
        ...
        try:
            if "patches" in plan and plan["patches"]:
                ...
                print(f"[file processor] Applied {len(plan['patches'])} patches to {file_path}")
                ...
                elif file_path in {"contracts/workflows/workflows.yaml", "contracts/app/commands.yaml"}:
                    safety_notes = _apply_auth_guard_param_safety_fixes(contracts_root, file_path)
                    for note in safety_notes:
        ...
        if post_validation_errors:
            for err in post_validation_errors[:3]:
                ...
                patch_errors.append(f"Post-patch validation failed at {location}: {message}")
        ...
        if success:
            ...
            print(f"[file processor] File repair: FAILED - {'; '.join(patch_errors[:2])}")
        ...
        return {
            ...
            "comprehensive_schema_modules": len(comprehensive_schema.get('modules', {})),
...
def _apply_rbac_repair_safety_fixes(contracts_root: Path) -> list[str]:
    ...
    from midicoder.contract.contract_utils import extract_ids_from_yaml_list, load_yaml_safe
    ...
    entity_ids = _load_ids_from_file(contracts_root / "domain" / "entities.yaml", "entities")
    command_ids = _load_ids_from_file(contracts_root / "app" / "commands.yaml", "commands")
    query_ids = _load_ids_from_file(contracts_root / "app" / "queries.yaml", "queries")
    projection_ids = _load_ids_from_file(contracts_root / "app" / "projections.yaml", "projections")
    ...
    policies_data = load_yaml_safe(policies_path.read_text(encoding="utf-8")) if policies_path.exists() else {}
    ...
    for permission in permissions:
        ...
        matching_policies = [
            policy for policy in policies if isinstance(policy, dict) and _policy_mentions_permission_id(policy, permission_id)
        ]
        if not matching_policies:
            ...
            notes.append(f"Added missing ownership policy for permission '{permission_id}'")
            continue

        if not any(_policy_has_ownership_condition_dict(policy) for policy in matching_policies):
...
def _policy_has_ownership_condition_dict(policy: dict[str, Any]) -> bool:
    ...
    for condition in conditions:
        ...
        if op != "eq":
            continue
        has_subject = any(token in field for token in ("owner", "employee_id", "user_id", "actor", "subject"))
        has_actor_value = any(
            token in value for token in ("actor", "subject", "user", "principal", "request", "current")
        )
...
def _scope_from_permission_resource(resource: str) -> str:
    ...
    kind, ref = [part.strip() for part in resource.split(":", 1)]
    kind = kind.lower()
    if kind in {"entity", "command", "query", "workflow", "projection", "integration"} and ref:
        return f"{kind}:{ref}"
    return "global"


def _apply_auth_guard_param_safety_fixes(contracts_root: Path, file_path: str) -> list[str]:
    ...
    def normalize_guard(guard: Any, location: str) -> None:
        ...
        if guard_id == "auth.permission":
            ...
            if isinstance(permission_value, list):
                ...
                changed = True
                notes.append(f"{location}: converted params.permission(list) -> params.permissions")
        ...
        for i, workflow in enumerate(workflows):
            ...
            for t, transition in enumerate(transitions):
                ...
                if not isinstance(guards, list):
                    continue
                for g, guard in enumerate(guards):
                    normalize_guard(guard, f"workflows[{i}].transitions[{t}].guards[{g}]")
