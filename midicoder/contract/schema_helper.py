...
def get_comprehensive_schema_for_file(*, schema_tree: dict, file_path: str) -> dict:
    ...
    try:
        ...
        for module_data in schema_cheatsheet["modules"].values():
            ...
            for key, value in module_catalogs.items():
                ...
                if isinstance(value, list):
                    ...
                    schema_cheatsheet["catalogs"][key] = sorted(str(item) for item in value)
                else:
        ...
        print(f"[schema extraction] Warning: Could not import slice_schema_for_targets: {import_exc}")
        return get_fallback_schema_for_file(schema_tree, file_path, f"Import error: {import_exc}")
    except Exception as exc:
        print(f"[schema extraction] Error: Failed to extract schema for {file_path}: {exc}")
        return get_fallback_schema_for_file(schema_tree, file_path, f"Extraction error: {exc}")


def get_file_cross_references(file_path: str, available_files: list[str] = None) -> dict[str, dict]:
    """
...
def get_fallback_schema_for_file(schema_tree: dict, file_path: str, error_msg: str) -> dict:
    """
    ...
    return {
        ...
        "repair_enhancements": {
            ...
            "error": error_msg,
            "schema_tree_keys": list(schema_tree.keys()) if isinstance(schema_tree, dict) else [],
            "extraction_summary": {
                "file_path": file_path,
                "modules_found": 0,
                "catalogs_found": 0,
