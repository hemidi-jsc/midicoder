...
from midicoder.packs.cp16_monitoring.parser import MonitoringParser
...
"""
        with pytest.raises(MidicoderError) as exc_info:
            self.parser.parse(dsl)
        assert exc_info.value.code == ErrorCode.CP16_INVALID_ALERT_CONDITION
...
"""
        with pytest.raises(MidicoderError) as exc_info:
            self.parser.parse(dsl)
        assert exc_info.value.code == ErrorCode.CP16_INVALID_ALERT_SEVERITY
...
"""
        with pytest.raises(MidicoderError) as exc_info:
            self.parser.parse(dsl)
        assert exc_info.value.code == ErrorCode.CP16_INVALID_SLI_METRIC_TYPE
...
"""
    ...
    def test_panel_to_dict(self):
        ...
        from midicoder.packs.cp16_monitoring.models import Panel
    ...
    def test_fired_alert_to_dict(self):
        ...
        from midicoder.packs.cp16_monitoring.models import FiredAlert
        import datetime
        fa = FiredAlert(
            rule_name="high-cpu",
            metric_name="cpu_usage",
    ...
    def test_sli_status_to_dict(self):
        ...
        from midicoder.packs.cp16_monitoring.models import SLIStatus
        import datetime
        ss = SLIStatus(
            sli_name="api-availability",
            metric_type="availability",
