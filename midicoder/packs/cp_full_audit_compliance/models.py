...
class AuditTrail:
    ...
    def __post_init__(self) -> None:
        ...
        if self.actor_type not in valid_actor_types:
            EM.raise_error(
                ErrorCode.MDC-F12_AUDIT_INVALID_ACTOR_TYPE,
        ...
        if not self.entity_type or not self.entity_type.strip():
            EM.raise_error(ErrorCode.MDC-F12_AUDIT_EMPTY_ID, field="entity_type")
        # Entity id không được để trống
        if not self.entity_id or not self.entity_id.strip():
            EM.raise_error(ErrorCode.MDC-F12_AUDIT_EMPTY_ID, field="entity_id")
...
class AuditRule:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-F12_AUDIT_EMPTY_ID, field="audit_rule.id")
        # Retention phải > 0
        if self.retention_days < 1:
            EM.raise_error(ErrorCode.MDC-F12_AUDIT_RETENTION_INVALID, days=self.retention_days)
        # Archive không được > retention
        if self.archive_after_days > self.retention_days:
            EM.raise_error(
                ErrorCode.MDC-F12_AUDIT_ARCHIVE_EXCEEDS_RETENTION,
...
class ComplianceControl:
    ...
    def __post_init__(self) -> None:
        ...
        # ID không được để trống
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-F12_AUDIT_EMPTY_ID, field="compliance_control.id")
...
class LedgerEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if self.sequence_number < 0:
            EM.raise_error(
                ErrorCode.MDC-F12_AUDIT_EMPTY_ID,
