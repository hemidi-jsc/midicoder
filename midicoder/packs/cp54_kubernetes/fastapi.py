# coding: utf-8
"""
FastAPI Emitter cho CP54: Kubernetes & Cloud Native Deployment.

Module này render Jinja2 templates để sinh Kubernetes manifest files
được pre-fill cho FastAPI application (port 8000, health check /health, etc.).
...
from midicoder.packs.cp54_kubernetes.models import (
    K8sDeployment,
    K8sEnvVar,
    K8sPort,
    K8sResources,
...
class FastAPIK8sEmitter:
    """Emitter cho FastAPI stack — CP54 Kubernetes.

    Tạo K8sIR với các giá trị mặc định phù hợp cho FastAPI app:
    - Port 8000
    - Health check /health
    - Resource defaults phù hợp cho Python/FastAPI
    """
    ...
    def emit(
        ...
        """Emit K8sIR được pre-fill cho FastAPI.

        Nếu ir đã có deployment, cập nhật với FastAPI defaults.
        ...
        Returns:
            K8sIR đã được pre-fill cho FastAPI.
        """
        if ir is None:
            ir = K8sIR()
        ...
        if not ir.deployments:
            deployment = K8sDeployment(
                ...
                ports=[K8sPort(container_port=8000, protocol="TCP", name="http")],
                resources=K8sResources(cpu="500m", memory="512Mi"),
                strategy="RollingUpdate",
                labels={"app": "fastapi-app"},
                env=[
                    K8sEnvVar(name="APP_HOST", value="0.0.0.0"),
                    K8sEnvVar(name="APP_PORT", value="8000"),
                    K8sEnvVar(name="APP_ENV", value="production"),
                ],
                volume_mounts=[],
            )
            ir.deployments.append(deployment)
        else:
            # Cập nhật deployment đầu tiên với FastAPI defaults
            dep = ir.deployments[0]
            if not dep.ports:
                dep.ports = [K8sPort(container_port=8000, protocol="TCP", name="http")]
            if not dep.labels:
                dep.labels = {"app": dep.name}

        # Nếu chưa có service, tạo mặc định
        if not ir.services:
            selector = ir.deployments[0].labels if ir.deployments[0].labels else {"app": "fastapi-app"}
            service = K8sService(
                id="fastapi-service",
                name="fastapi-app",
                type="ClusterIP",
...
{ports_yaml.rstrip()}
          ...
          livenessProbe:
            httpGet:
              path: /health
              port: {deployment.ports[0].container_port if deployment.ports else 8000}
