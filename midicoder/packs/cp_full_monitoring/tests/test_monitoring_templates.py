...
class TestTemplateExistence:
    ...
    def test_fastapi_template_exists(self, template) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "fastapi", "core", "cp_full_monitoring", template)
    ...
    def test_nestjs_template_exists(self, template) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "nestjs", "core", "cp_full_monitoring", template)
        assert os.path.isfile(path), f"Template không tồn tại: {path}"

    @pytest.mark.parametrize("template", TEMPLATES["angular"])
    def test_angular_template_exists(self, template) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "angular", "core", "cp_full_monitoring", template)
        assert os.path.isfile(path), f"Template không tồn tại: {path}"

    @pytest.mark.parametrize("template", TEMPLATES["react"])
    def test_react_template_exists(self, template) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "react", "core", "cp_full_monitoring", template)
        assert os.path.isfile(path), f"Template không tồn tại: {path}"
...
class TestTemplateContent:
    ...
    def test_fastapi_service_has_monitoring_service(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "fastapi", "core", "cp_full_monitoring", "monitoring_service.py.jinja2")
        content = open(path, encoding="utf-8").read()
        assert "MonitoringService" in content or "monitoring_service" in content

    def test_fastapi_router_has_router(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "fastapi", "core", "cp_full_monitoring", "monitoring_router.py.jinja2")
        content = open(path, encoding="utf-8").read()
        assert "APIRouter" in content or "router" in content.lower()

    def test_nestjs_module_has_module_decorator(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "nestjs", "core", "cp_full_monitoring", "monitoring.module.ts.jinja2")
        content = open(path, encoding="utf-8").read()
        assert "@Module" in content or "Module" in content

    def test_nestjs_service_has_service_class(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "nestjs", "core", "cp_full_monitoring", "monitoring.service.ts.jinja2")
        content = open(path, encoding="utf-8").read()
        assert "MonitoringService" in content or "Service" in content

    def test_nestjs_controller_has_controller(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "nestjs", "core", "cp_full_monitoring", "monitoring.controller.ts.jinja2")
        content = open(path, encoding="utf-8").read()
        assert "MonitoringController" in content or "Controller" in content

    def test_angular_dashboard_has_component(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "angular", "core", "cp_full_monitoring", "dashboard_widget.component.ts.jinja2")
        content = open(path, encoding="utf-8").read()
        assert "@Component" in content or "Component" in content

    def test_angular_alert_has_panel(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "angular", "core", "cp_full_monitoring", "alert_panel.component.ts.jinja2")
        content = open(path, encoding="utf-8").read()
        assert "@Component" in content or "Component" in content

    def test_react_types_has_interface(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "react", "core", "cp_full_monitoring", "types.ts.jinja2")
        content = open(path, encoding="utf-8").read()
        assert "interface" in content or "type" in content

    def test_react_dashboard_has_function_component(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "react", "core", "cp_full_monitoring", "DashboardPanel.tsx.jinja2")
        content = open(path, encoding="utf-8").read()
        assert "DashboardPanel" in content or "function" in content or "const" in content

    def test_react_alert_has_banner(self) -> None:
        path = os.path.join(REPO_ROOT, "midicoder", "stacks", "react", "core", "cp_full_monitoring", "AlertBanner.tsx.jinja2")
        content = open(path, encoding="utf-8").read()
    ...
    def test_all_templates_non_empty(self) -> None:
        for stack, templates in TEMPLATES.items():
            for template in templates:
                path = os.path.join(REPO_ROOT, "midicoder", "stacks", stack, "core", "cp_full_monitoring", template)
