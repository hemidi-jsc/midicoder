...
Ki?m tra:
- MetricType: Gi tr? enum v membership
- LogLevel: Gi tr? enum v membership
- TracePropagationFormat: Gi tr? enum v membership
- MetricProfile: T?o h?p l?, tn r?ng ? l?i, retention < 1 ? l?i, to_dict/from_dict, defaults
- StructuredLogConfig: T?o h?p l?, format sai ? l?i, defaults, to_dict/from_dict
- TraceConfig: T?o h?p l?, max_spans < 1 ? l?i, sample_rate ra ngoi kho?ng ? l?i, defaults, to_dict/from_dict
"""
...
class TestMetricType:
    ...
    def test_counter_value(self):
        """Ki?m tra gi tr? COUNTER."""
        assert MetricType.COUNTER.value == "counter"

    def test_gauge_value(self):
        """Ki?m tra gi tr? GAUGE."""
        assert MetricType.GAUGE.value == "gauge"

    def test_histogram_value(self):
        """Ki?m tra gi tr? HISTOGRAM."""
        assert MetricType.HISTOGRAM.value == "histogram"

    def test_total_count(self):
        """Ki?m tra t?ng s? lo?i metric = 3."""
        assert len(MetricType) == 3

    def test_string_comparison(self):
        """Ki?m tra so snh v?i string."""
...
class TestLogLevel:
    ...
    def test_warning_value(self):
        """Ki?m tra gi tr? WARNING."""
        assert LogLevel.WARNING.value == "WARNING"

    def test_error_value(self):
        """Ki?m tra gi tr? ERROR."""
        assert LogLevel.ERROR.value == "ERROR"

    def test_critical_value(self):
        """Ki?m tra gi tr? CRITICAL."""
        assert LogLevel.CRITICAL.value == "CRITICAL"

    def test_total_count(self):
        """Ki?m tra t?ng s? log levels = 5."""
...
class TestTracePropagationFormat:
    ...
    def test_w3c_value(self):
        """Ki?m tra gi tr? W3C_TRACE_CONTEXT."""
    ...
    def test_total_count(self):
        """Ki?m tra t?ng s? propagation formats = 3."""
...
class TestMetricProfile:
    ...
    def test_create_valid_counter(self):
        """Ki?m tra t?o counter metric h?p l?."""
    ...
    def test_create_valid_gauge(self):
        """Ki?m tra t?o gauge metric h?p l?."""
        mp = MetricProfile(
            name="memory_usage",
        ...
        assert mp.description == "Theo di b? nh?"

    def test_create_valid_histogram(self):
        """Ki?m tra t?o histogram metric h?p l?."""
        mp = MetricProfile(
    ...
    def test_empty_name_raises_error(self):
        """Ki?m tra tn metric r?ng throw error."""
        with pytest.raises(MidicoderError) as exc_info:
            MetricProfile(name="")
        assert exc_info.value.code == ErrorCode.MDC-C03_EMPTY_METRIC_NAME

    def test_whitespace_only_name_raises_error(self):
        """Ki?m tra tn metric ch? whitespace throw error."""
        with pytest.raises(MidicoderError) as exc_info:
            MetricProfile(name="   ")
        assert exc_info.value.code == ErrorCode.MDC-C03_EMPTY_METRIC_NAME

    def test_zero_retention_raises_error(self):
        """Ki?m tra retention_days = 0 throw error."""
        with pytest.raises(MidicoderError) as exc_info:
            MetricProfile(name="test", retention_days=0)
        assert exc_info.value.code == ErrorCode.MDC-C03_METRIC_RETENTION_INVALID

    def test_negative_retention_raises_error(self):
        """Ki?m tra retention_days m throw error."""
        with pytest.raises(MidicoderError) as exc_info:
            MetricProfile(name="test", retention_days=-5)
        assert exc_info.value.code == ErrorCode.MDC-C03_METRIC_RETENTION_INVALID

    def test_default_values(self):
        """Ki?m tra default values c?a MetricProfile."""
    ...
    def test_to_dict(self):
        """Ki?m tra to_dict serialization."""
        mp = MetricProfile(
            name="cpu_usage",
            metric_type=MetricType.GAUGE,
            unit="percent",
        ...
        assert data["description"] == "Theo di CPU"

    def test_from_dict(self):
        """Ki?m tra from_dict deserialization."""
        data = {
            "name": "disk_io",
    ...
    def test_roundtrip_to_dict_from_dict(self):
        """Ki?m tra roundtrip to_dict ? from_dict."""
...
class TestStructuredLogConfig:
    ...
    def test_create_valid_json(self):
        """Ki?m tra t?o log config h?p l? v?i format json."""
    ...
    def test_create_valid_text(self):
        """Ki?m tra t?o log config h?p l? v?i format text."""
    ...
    def test_invalid_format_raises_error(self):
        """Ki?m tra output_format khng h?p l? throw error."""
        with pytest.raises(MidicoderError) as exc_info:
            StructuredLogConfig(service_name="test", output_format="xml")
        assert exc_info.value.code == ErrorCode.MDC-C03_OBSERVABILITY_PARSE_ERROR

    def test_all_log_levels_valid(self):
        """Ki?m tra t?t c? log levels d?u h?p l?."""
        for level in LogLevel:
            cfg = StructuredLogConfig(service_name="test", log_level=level)
            assert cfg.log_level == level

    def test_default_values(self):
        """Ki?m tra default values c?a StructuredLogConfig."""
        cfg = StructuredLogConfig()
    ...
    def test_compute_entry_hash(self):
        """Ki?m tra compute_entry_hash tr? v? SHA-256."""
    ...
    def test_compute_entry_hash_different_messages(self):
        """Ki?m tra hash khc nhau cho message khc nhau."""
        cfg = StructuredLogConfig(service_name="test-svc")
...
class TestTraceConfig:
    ...
    def test_create_valid_w3c(self):
        """Ki?m tra t?o trace config h?p l? v?i W3C."""
    ...
    def test_create_valid_b3(self):
        """Ki?m tra t?o trace config h?p l? v?i B3."""
        cfg = TraceConfig(
            service_name="payment",
    ...
    def test_create_valid_none(self):
        """Ki?m tra t?o trace config h?p l? v?i NONE."""
        cfg = TraceConfig(
    ...
    def test_custom_attributes(self):
        """Ki?m tra thm custom attributes."""
        cfg = TraceConfig(
            service_name="api",
...
class TestLogDestinationType:
    ...
    def test_total_count(self):
        """Ki?m tra t?ng s? destination types = 7."""
...
class TestOTelExporterConfig:
    ...
    def test_create_valid_defaults(self):
        """Ki?m tra t?o exporter config h?p l? v?i defaults."""
    ...
    def test_create_valid_custom(self):
        """Ki?m tra t?o exporter config custom."""
        cfg = OTelExporterConfig(
...
class TestOpenTelemetryConfig:
    ...
    def test_create_valid_defaults(self):
        """Ki?m tra t?o OpenTelemetry config h?p l? v?i defaults."""
    ...
    def test_create_valid_full(self):
        """Ki?m tra t?o OpenTelemetry config d?y d?."""
    ...
    def test_from_dict_without_exporter(self):
        """Ki?m tra from_dict khng c exporter dng default."""
        data = {
...
class TestLogShippingDestination:
    ...
    def test_create_valid_loki(self):
        """Ki?m tra t?o destination Loki h?p l?."""
    ...
    def test_negative_flush_interval_clamped(self):
        """Ki?m tra flush_interval_ms m du?c clamp."""
        dest = LogShippingDestination(
            destination_type=LogDestinationType.LOKI,
            flush_interval_ms=-1,
        )
    ...
    def test_zero_batch_size_clamped(self):
        """Ki?m tra max_batch_size = 0 du?c clamp."""
        dest = LogShippingDestination(
            destination_type=LogDestinationType.LOKI,
            max_batch_size=0,
        )
...
class TestLogShippingConfig:
    ...
    def test_create_valid_defaults(self):
        """Ki?m tra t?o config h?p l? v?i defaults."""
    ...
    def test_create_valid_with_destinations(self):
        """Ki?m tra t?o config v?i destinations."""
        dests = [
    ...
    def test_zero_queue_size_clamped(self):
        """Ki?m tra max_queue_size = 0 du?c clamp v? 10000."""
    ...
    def test_from_dict_empty_destinations(self):
        """Ki?m tra from_dict khng c destinations."""
        data = {"enabled": True}
        cfg = LogShippingConfig.from_dict(data)
        assert cfg.enabled is True
        assert cfg.destinations == []
