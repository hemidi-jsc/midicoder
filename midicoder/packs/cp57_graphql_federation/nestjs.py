# coding: utf-8
"""
NestJS Emitter cho CP57: GraphQL Schema Federation.

Module này render Jinja2 templates để sinh GraphQL Federation code
cho NestJS stack, bao gồm federation module, resolvers, entities,
...
class NestJSFederationEmitter:
    """Emitter cho NestJS stack — CP57 GraphQL Schema Federation.

    Render templates từ `stacks/nestjs/cp57_graphql_federation/`
    ...
    def __init__(self, stack_dir: str | Path) -> None:
        ...
        self.template_dir = self.stack_dir / "cp57_graphql_federation"

        if not self.template_dir.exists():
            raise EM.raise_error(
                ErrorCode.CP57_FEDERATION_SERVICE_NOT_FOUND,
    ...
    def emit(
        ...
        """Emit GraphQL Federation code cho NestJS.

        Sinh các files:
        - graphql-federation.module.ts (federation module)
        - graphql.resolvers.ts (resolver classes)
        - graphql.entities.ts (entity type definitions)
        - gateway-config.ts (gateway aggregation config)
        ...
        templates = [
            ("graphql-federation.module.ts.jinja2", "src/graphql/graphql-federation.module.ts"),
            ("graphql.resolvers.ts.jinja2", "src/graphql/graphql.resolvers.ts"),
            ("graphql.entities.ts.jinja2", "src/graphql/graphql.entities.ts"),
            ("gateway-config.ts.jinja2", "src/graphql/gateway-config.ts"),
        ]
    ...
    def _render(self, template_name: str, context: dict[str, Any]) -> str:
        try:
            ...
            raise EM.raise_error(
                ErrorCode.CP57_INVALID_GATEWAY_CONFIG,
