...
class TestFastAPIEmitter:
    ...
    def test_emit_file_count(self, tmp_path: Path) -> None:
        ...
        for template_name, _ in FASTAPI_TEMPLATES:
            try:
                content = emitter.render(f"cp27_plugin_system/{template_name}", context)
    ...
    def test_emit_init_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="fastapi")
        content = emitter.render("cp27_plugin_system/__init__.py.jinja2", _make_context())
    ...
    def test_emit_slots_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="fastapi")
        content = emitter.render("cp27_plugin_system/slots.py.jinja2", _make_context())
    ...
    def test_emit_contracts_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="fastapi")
        content = emitter.render("cp27_plugin_system/contracts.py.jinja2", _make_context())
    ...
    def test_emit_policies_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="fastapi")
        content = emitter.render("cp27_plugin_system/policies.py.jinja2", _make_context())
        assert "policy" in content.lower()

    def test_emit_loader_file(self, tmp_path: Path) -> None:
        """loader.py chứa discover/load logic."""
        emitter = Emitter(stack="fastapi")
        content = emitter.render("cp27_plugin_system/loader.py.jinja2", _make_context())
    ...
    def test_emit_registry_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="fastapi")
        content = emitter.render("cp27_plugin_system/registry.py.jinja2", _make_context())
    ...
    def test_emit_manager_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="fastapi")
        content = emitter.render("cp27_plugin_system/manager.py.jinja2", _make_context())
    ...
    def test_emit_api_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="fastapi")
        content = emitter.render("cp27_plugin_system/api.py.jinja2", _make_context())
        assert "plugin" in content.lower()

    def test_emit_middleware_file(self, tmp_path: Path) -> None:
        """middleware.py chứa middleware class."""
        emitter = Emitter(stack="fastapi")
        content = emitter.render("cp27_plugin_system/middleware.py.jinja2", _make_context())
    ...
    def test_emit_example_and_json(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="fastapi")
        example = emitter.render("cp27_plugin_system/example_plugin.py.jinja2", _make_context())
        assert "plugin" in example.lower()

        manifest = emitter.render("cp27_plugin_system/plugin.json.jinja2", _make_context())
...
class TestNestJSEmitter:
    ...
    def test_emit_module_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="nestjs")
        content = emitter.render("cp27_plugin_system/plugins.module.ts.jinja2", _make_context())
        assert "@Module" in content or "Module" in content

    def test_emit_slots_file(self, tmp_path: Path) -> None:
        """slots.ts chứa enum PluginSlot."""
        emitter = Emitter(stack="nestjs")
        content = emitter.render("cp27_plugin_system/slots.ts.jinja2", _make_context())
        assert "enum" in content or "PluginSlot" in content

    def test_emit_contracts_file(self, tmp_path: Path) -> None:
        """contracts.ts chứa interface."""
        emitter = Emitter(stack="nestjs")
        content = emitter.render("cp27_plugin_system/contracts.ts.jinja2", _make_context())
        assert "interface" in content or "IPlugin" in content

    def test_emit_loader_file(self, tmp_path: Path) -> None:
        """plugin-loader.ts chứa load logic."""
        emitter = Emitter(stack="nestjs")
        content = emitter.render("cp27_plugin_system/plugin-loader.ts.jinja2", _make_context())
        assert "load" in content.lower()

    def test_emit_registry_file(self, tmp_path: Path) -> None:
        """plugin-registry.service.ts chứa registry."""
        emitter = Emitter(stack="nestjs")
        content = emitter.render("cp27_plugin_system/plugin-registry.service.ts.jinja2", _make_context())
        assert "registry" in content.lower() or "register" in content.lower()

    def test_emit_manager_file(self, tmp_path: Path) -> None:
        """plugin-manager.service.ts chứa manager."""
        emitter = Emitter(stack="nestjs")
        content = emitter.render("cp27_plugin_system/plugin-manager.service.ts.jinja2", _make_context())
        assert "manager" in content.lower() or "enable" in content.lower()

    def test_emit_controller_file(self, tmp_path: Path) -> None:
        """plugin.controller.ts chứa @Controller."""
        emitter = Emitter(stack="nestjs")
        content = emitter.render("cp27_plugin_system/plugin.controller.ts.jinja2", _make_context())
        assert "@Controller" in content or "Controller" in content

    def test_emit_middleware_file(self, tmp_path: Path) -> None:
        """plugin.middleware.ts chứa middleware."""
        emitter = Emitter(stack="nestjs")
        content = emitter.render("cp27_plugin_system/plugin.middleware.ts.jinja2", _make_context())
        assert "middleware" in content.lower()

    def test_emit_example_and_json(self, tmp_path: Path) -> None:
        """example.plugin.ts và plugin.json render được."""
        emitter = Emitter(stack="nestjs")
        example = emitter.render("cp27_plugin_system/example.plugin.ts.jinja2", _make_context())
        assert "plugin" in example.lower()
...
class TestAngularEmitter:
    ...
    def test_emit_module_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="angular")
        content = emitter.render("cp27_plugin_system/plugin.module.ts.jinja2", _make_context())
        assert "@NgModule" in content or "NgModule" in content

    def test_emit_contracts_file(self, tmp_path: Path) -> None:
        """plugin-contracts.ts chứa interface."""
        emitter = Emitter(stack="angular")
        content = emitter.render("cp27_plugin_system/plugin-contracts.ts.jinja2", _make_context())
        assert "interface" in content or "IPlugin" in content

    def test_emit_api_service_file(self, tmp_path: Path) -> None:
        """plugin-api.service.ts chứa service/HttpClient."""
        emitter = Emitter(stack="angular")
        content = emitter.render("cp27_plugin_system/plugin-api.service.ts.jinja2", _make_context())
        assert "service" in content.lower() or "HttpClient" in content

    def test_emit_registry_file(self, tmp_path: Path) -> None:
        """plugin-registry.ts chứa registry."""
        emitter = Emitter(stack="angular")
        content = emitter.render("cp27_plugin_system/plugin-registry.ts.jinja2", _make_context())
        assert "registry" in content.lower() or "register" in content.lower()
...
class TestReactEmitter:
    ...
    def test_emit_api_file(self, tmp_path: Path) -> None:
        ...
        emitter = Emitter(stack="react")
        content = emitter.render("cp27_plugin_system/plugin-api.ts.jinja2", _make_context())
...
class TestErrorBoundary:
    ...
    def test_emitter_raises_error_for_missing_template(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(Exception):
            emitter.render("cp27_plugin_system/nonexistent_template.py.jinja2", _make_context())
