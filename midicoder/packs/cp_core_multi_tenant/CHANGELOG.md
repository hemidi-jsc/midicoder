# Changelog — C01 Multi-Tenant Architecture (Core Pack)
...
- **Pack rename**: CP02 → C01 (taxonomy-v2 migration)
- **Internal ID**: `cp02_multi_tenant` → `cp_core_multi_tenant`
- **Category**: `architecture` → `core`
- **Type**: added `type: core`
- **Dependencies**: `CP01` → `B01, F07` (added F07 per CP19 wiring rule)
- **Capabilities provided**: renamed `enforce_tenant_scope, tenant_isolation` → `tenant_middleware, tenant_context` (aligned with runtime capability IDs)
- **All imports**: updated from `midicoder.packs.cp02_multi_tenant` → `midicoder.packs.cp_core_multi_tenant`
- **Template paths**: updated from `cp02_multi_tenant/` → `cp_core_multi_tenant/`
- **Stack directories**: renamed `cp02_multi_tenant/` → `cp_core_multi_tenant/` (fastapi, nestjs)
- **Render context**: added `render_context_support: true`
- **Old IDs**: added `old_ids: [CP02]` for backward compatibility
