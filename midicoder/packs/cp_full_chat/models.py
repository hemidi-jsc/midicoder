...
class Conversation:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.conversation_id or not self.conversation_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F19_CHAT_CONVERSATION_NOT_FOUND,
...
class ChatMessage:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.message_id or not self.message_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F19_CHAT_MESSAGE_TOO_LONG,
        ...
        if not self.sender_id or not self.sender_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F19_CHAT_USER_NOT_IN_CONVERSATION,
...
class ReadReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.receipt_id or not self.receipt_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F19_CHAT_NOTIFICATION_SEND_FAILED,
