    ...
    def __post_init__(self) -> None:
        ...
        if self.cron_expr is None and self.interval_seconds is None:
            raise ValueError(
                ...
                f"[{ErrorCode.MDC-F20_SCHEDULE_POLICY_INVALID.value}]"
            )
...
class JobDefinition:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.job_id or not self.job_id.strip():
            raise ValueError(
                f"job_id không được rỗng [{ErrorCode.MDC-F20_JOB_NOT_FOUND.value}]"
            )
        if not self.task_type or not self.task_type.strip():
            raise ValueError(
                f"task_type không được rỗng [{ErrorCode.MDC-F20_JOB_SCHEDULE_FAILED.value}]"
            )
...
class JobInstance:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.instance_id or not self.instance_id.strip():
            raise ValueError(
                f"instance_id không được rỗng [{ErrorCode.MDC-F20_JOB_NOT_FOUND.value}]"
            )
        if not self.job_id or not self.job_id.strip():
            raise ValueError(
                f"job_id không được rỗng [{ErrorCode.MDC-F20_JOB_NOT_FOUND.value}]"
...
class JobRetryTracker:
    ...
    def record_failure(self, error: str) -> None:
        ...
        if self.current_attempt >= self.retry_policy.max_retries:
            ...
            EM.raise_error(
                ErrorCode.MDC-F20_JOB_RETRY_EXHAUSTED,
