...
# ===========================================================================
# Scheduler Emitter (from CP31)
# ===========================================================================

from dataclasses import dataclass as nj_dp_dataclass
from pathlib import Path as NJ_Path
from typing import Any as NJ_Any

from jinja2 import Environment as NJ_Env, FileSystemLoader as NJ_FSL
...
class SchedulerNestJSEmitter:
    """Emitter cho NestJS stack — Scheduler & Cron Engine (từ CP31).

    Render templates từ `stacks/nestjs/cp31_scheduler/`
    để sinh scheduler engine code.
    """

    def __init__(self, stack_dir: str | NJ_Path) -> None:
        """Initialise emitter.

        Args:
            stack_dir: Đường dẫn đến `stacks/nestjs/`.
        """
        self.stack_dir = NJ_Path(stack_dir)
        ...
        self.env = NJ_Env(
            loader=NJ_FSL(str(self.template_dir)),
            autoescape=True,
            trim_blocks=True,
            lstrip_blocks=True,
        )
    ...
    def emit(
        self,
        schedules: list[dict[str, NJ_Any]],
        output_dir: str | NJ_Path,
        calendar_sets: list[dict[str, NJ_Any]] | None = None,
    ) -> list[GeneratedFile]:
    ...
    def _render(self, template_name: str, context: dict[str, NJ_Any]) -> str:
