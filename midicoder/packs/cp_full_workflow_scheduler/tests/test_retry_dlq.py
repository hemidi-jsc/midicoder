...
from midicoder.packs.cp_full_workflow_scheduler.scheduler import RetryPolicy, DeadLetterQueue, JobRetryTracker
from midicoder.packs.cp_full_workflow_scheduler.models import WorkflowDefinition, Transition, GuardType, EffectType, Guard, Effect
from midicoder.packs.cp_full_workflow_scheduler.engine.state_machine import StateMachine
...
class TestJobRetryTracker(TestCase):
    ...
    def test_raises_cp13_job_retry_exhausted_on_exhaustion(self):
        ...
        with self.assertRaises(MidicoderError) as ctx:
            tracker.record_failure("err2")
        self.assertIn(ErrorCode.MDC-F20_JOB_RETRY_EXHAUSTED.value, str(ctx.exception))
...
class TestStateMachineEdgeCases(TestCase):
    ...
    def test_invalid_event_raises_error(self):
        ...
        self.assertIn(ErrorCode.MDC-B01_WORKFLOW_INVALID_TRANSITION.value, str(ctx.exception))
    ...
    def test_instance_not_found_raises_error(self):
        ...
        self.assertIn(ErrorCode.MDC-B01_WORKFLOW_NOT_FOUND.value, str(ctx.exception))
    ...
    def test_already_in_target_state_raises_error(self):
        ...
        self.assertIn(ErrorCode.MDC-B01_WORKFLOW_ALREADY_IN_STATE.value, str(ctx.exception))
