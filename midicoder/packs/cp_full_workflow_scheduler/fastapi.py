...
from dataclasses import dataclass as dp_dataclass, field as dp_field
from pathlib import Path as DP_Path
from typing import Any as DP_Any

from jinja2 import Environment as DP_Env, FileSystemLoader as DP_FSL
...
class SchedulerFastAPIEmitter:
    """Emitter cho FastAPI stack — Scheduler & Cron Engine (từ CP31).

    Render templates từ `stacks/fastapi/cp31_scheduler/`
    để sinh scheduler engine code.
    """

    def __init__(self, stack_dir: str | DP_Path) -> None:
        """Initialise emitter.

        Args:
            stack_dir: Đường dẫn đến `stacks/fastapi/`.
        """
        self.stack_dir = DP_Path(stack_dir)
        ...
        self.env = DP_Env(
            loader=DP_FSL(str(self.template_dir)),
            autoescape=True,
            trim_blocks=True,
            lstrip_blocks=True,
        )
    ...
    def emit(
        self,
        schedules: list[dict[str, DP_Any]],
        output_dir: str | DP_Path,
        calendar_sets: list[dict[str, DP_Any]] | None = None,
    ) -> list[GeneratedFile]:
        ...
        if self._template_exists("cron_parser.py.jinja2"):
            ...
            files.append(GeneratedFile(path="app/scheduler/cron_parser.py", content=content))

        if self._template_exists("calendar_engine.py.jinja2"):
            content = self._render("calendar_engine.py.jinja2", context)
            files.append(GeneratedFile(path="app/scheduler/calendar_engine.py", content=content))

        if self._template_exists("scheduler_service.py.jinja2"):
            content = self._render("scheduler_service.py.jinja2", context)
            files.append(GeneratedFile(path="app/scheduler/scheduler_service.py", content=content))

        if self._template_exists("scheduler_router.py.jinja2"):
            content = self._render("scheduler_router.py.jinja2", context)
            files.append(GeneratedFile(path="app/scheduler/scheduler_router.py", content=content))
    ...
    def _render(self, template_name: str, context: dict[str, DP_Any]) -> str:
