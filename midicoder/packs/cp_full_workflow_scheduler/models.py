    ...
    def _validate(self) -> None:
        ...
        if self.type == GuardType.PERMISSION:
            if not self.permission:
                EM.raise_error(
                    ErrorCode.MDC-B01_WORKFLOW_GUARD_FAILED,
...
class WorkflowDefinition:
    ...
    def _validate(self) -> None:
        ...
        if not self.states:
            EM.raise_error(
                ErrorCode.MDC-B01_WORKFLOW_INVALID_STATE,
        ...
        for transition in self.transitions:
            if transition.from_state not in self.states:
                EM.raise_error(
                    ErrorCode.MDC-B01_WORKFLOW_INVALID_TRANSITION,
                    workflow_name=self.name,
...
class JobSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.job_id:
            EM.raise_error(
                ErrorCode.MDC-F20_JOB_SPEC_INVALID,
...
class WorkerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.pool_size <= 0:
            EM.raise_error(
                ErrorCode.MDC-F20_WORKER_CONFIG_INVALID,
...
class DeadlockDetection:
    ...
    def __post_init__(self) -> None:
        ...
        if self.on_deadlock not in valid_strategies:
            EM.raise_error(
                ErrorCode.MDC-F20_DEADLOCK_CONFIG_INVALID,
...
# ===========================================================================
# Scheduler & Cron Engine models (từ CP31)
# ===========================================================================
    ...
    def __post_init__(self) -> None:
        ...
        if not self.schedule_id or not self.schedule_id.strip():
            raise EM.raise_error(
                ErrorCode.MDC-F20_INVALID_CRON_EXPR,
...
class RecurrenceSchedule:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.schedule_id or not self.schedule_id.strip():
            raise EM.raise_error(
                ErrorCode.MDC-F20_INVALID_RECURRENCE,
                schedule_id=self.schedule_id,
                reason="schedule_id không được rỗng",
            )
        if self.interval < 1:
...
class CalendarSchedule:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.calendar_set_id or not self.calendar_set_id.strip():
            raise EM.raise_error(
                ErrorCode.MDC-F20_CALENDAR_NOT_FOUND,
                schedule_id=self.schedule_id,
...
class CalendarException:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.exception_id or not self.exception_id.strip():
            raise EM.raise_error(
                ErrorCode.MDC-F20_BUSINESS_DAY_ERROR,
