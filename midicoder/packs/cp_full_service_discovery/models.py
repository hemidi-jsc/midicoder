...
class TargetingRule:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.rule_id or not self.rule_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F33_INVALID_TARGETING_RULE,
...
class FeatureFlag:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.flag_key or not self.flag_key.strip():
            EM.raise_error(ErrorCode.MDC-F33_EMPTY_FLAG_KEY, message="Flag key không được để trống")

        if not re.match(r'^[a-z][a-z0-9_]*$', self.flag_key):
            EM.raise_error(ErrorCode.MDC-F33_EMPTY_FLAG_KEY, message=f"Flag key phải là snake_case: {self.flag_key}")

        if self.variant_type == FlagVariantType.PERCENTAGE:
            if self.percentage < 0 or self.percentage > 100:
                EM.raise_error(
                    ErrorCode.MDC-F33_INVALID_PERCENTAGE,
...
class ABVariant:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.variant_key or not self.variant_key.strip():
            EM.raise_error(ErrorCode.MDC-F33_INVALID_VARIANT_WEIGHT,
...
class ABExperiment:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.experiment_key or not self.experiment_key.strip():
            EM.raise_error(ErrorCode.MDC-F33_EMPTY_EXPERIMENT_KEY,
                           message="Experiment key không được để trống")

        if len(self.variants) < 2:
            EM.raise_error(ErrorCode.MDC-F33_TOO_FEW_VARIANTS,
...
class ABExperimentEngine:
    ...
    def add_experiment(self, experiment: ABExperiment) -> None:
        ...
        if experiment.experiment_key in self.experiments:
            EM.raise_error(
                ErrorCode.MDC-F33_DUPLICATE_EXPERIMENT_KEY,
    ...
    def assign(self, experiment_key: str, user_id: str) -> ABVariant | None:
        ...
        if not experiment:
            EM.raise_error(
                ErrorCode.MDC-F33_EXPERIMENT_NOT_FOUND,
...
class DynamicConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.config_key or not self.config_key.strip():
            EM.raise_error(ErrorCode.MDC-F33_EMPTY_CONFIG_KEY, message="Config key không được để trống")
...
# Service Discovery models merged from CP60
