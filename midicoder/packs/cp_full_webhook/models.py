...
class WebhookAuthConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.secret or not self.secret.strip():
            EM.raise_error(
                ErrorCode.MDC-F17_WEBHOOK_AUTH_CONFIG_INVALID,
...
class WebhookSubscription:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.subscription_id or not self.subscription_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F17_WEBHOOK_SUBSCRIPTION_NOT_FOUND,
        ...
        if not self._is_valid_url(self.url):
            EM.raise_error(
                ErrorCode.MDC-F17_WEBHOOK_INVALID_URL,
                url=self.url,
            )
...
class WebhookDispatcher:
    ...
    def compute_hmac_signature(self, payload: dict[str, Any], secret: str, algorithm: str = "sha256") -> str:
        ...
        try:
            ...
            EM.raise_error(
                ErrorCode.MDC-F17_WEBHOOK_SIGNATURE_GENERATION_FAILED,
                reason=str(e),
            )
