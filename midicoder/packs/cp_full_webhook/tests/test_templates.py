...
FASTAPI_DIR = MIDICODER_ROOT / "stacks" / "fastapi" / "core" / "cp_full_webhook"
NESTJS_DIR = MIDICODER_ROOT / "stacks" / "nestjs" / "core" / "cp_full_webhook"
ANGULAR_DIR = MIDICODER_ROOT / "stacks" / "angular" / "core" / "cp_full_webhook"
REACT_DIR = MIDICODER_ROOT / "stacks" / "react" / "core" / "cp_full_webhook"
PACK_DIR = MIDICODER_ROOT / "emitters" / "core" / "cp_full_webhook"
...
class TestPackYaml:
    ...
    def test_pack_yml_has_cp40_id(self):
        ...
        assert "cp_full_webhook" in content
...
class TestRegistry:
    ...
    def test_cp40_in_registry(self):
        ...
        assert CP_ID_TO_INTERNAL["CP40"] == "cp_full_webhook"
...
FASTAPI_TEMPLATES_40 = TestFastAPITemplates.EXPECTED
NESTJS_TEMPLATES_40 = TestNestJSTemplates.EXPECTED
ANGULAR_TEMPLATES_40 = TestAngularTemplates.EXPECTED
REACT_TEMPLATES_40 = TestReactTemplates.EXPECTED

ALL_TEMPLATES_40 = {
    "fastapi": FASTAPI_TEMPLATES_40,
    "nestjs": NESTJS_TEMPLATES_40,
    "angular": ANGULAR_TEMPLATES_40,
    "react": REACT_TEMPLATES_40,
}


def _render_template_40(stack: str, template_name: str) -> str:
    """Render template với context cơ bản; fallback sang đọc source thô nếu render lỗi."""
    env = jinja2.Environment(
        loader=jinja2.FileSystemLoader(str(STACK_DIRS_40[stack])),
    ...
    try:
        ...
        source_path = STACK_DIRS_40[stack] / template_name
...
class TestRuleV1NoMidicoderImport:
    ...
    def test_fastapi_no_midicoder_import(self):
        ...
        for template in FASTAPI_TEMPLATES_40:
            result = _render_template_40("fastapi", template)
    ...
    def test_nestjs_no_midicoder_import(self):
        ...
        for template in NESTJS_TEMPLATES_40:
            result = _render_template_40("nestjs", template)
            assert "from midicoder" not in result, f"Rule V1 vi phạm: {template}"
            assert "import midicoder" not in result, f"Rule V1 vi phạm: {template}"

    def test_angular_no_midicoder_import(self):
        """Angular templates không chứa 'from midicoder' trong output."""
        for template in ANGULAR_TEMPLATES_40:
            result = _render_template_40("angular", template)
            assert "from midicoder" not in result, f"Rule V1 vi phạm: {template}"
            assert "import midicoder" not in result, f"Rule V1 vi phạm: {template}"

    def test_react_no_midicoder_import(self):
        """React templates không chứa 'from midicoder' trong output."""
        for template in REACT_TEMPLATES_40:
            result = _render_template_40("react", template)
            assert "from midicoder" not in result, f"Rule V1 vi phạm: {template}"
            assert "import midicoder" not in result, f"Rule V1 vi phạm: {template}"
