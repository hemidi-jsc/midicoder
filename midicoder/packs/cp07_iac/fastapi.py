"""
FastAPI IAC Emitter — Emit Docker Compose cho FastAPI stack.

Module này cung cấp FastAPIIacEmitter class để generate
Docker Compose files cho FastAPI backend.

Sử dụng:
    from midicoder.packs.cp07_iac.fastapi import FastAPIIacEmitter
...
class FastAPIIacEmitter:
    """
    FastAPI Infrastructure as Code Emitter.

    Emitter này wrapper quanh DockerComposeGenerator để
    generate docker-compose.yml cho FastAPI stack.
    ...
    def __init__(self) -> None:
        """Initialize FastAPIIacEmitter."""
        self.generator = DockerComposeGenerator()

    def generate(self, config: InfrastructureConfig) -> dict[str, str]:
        """
        Generate Docker Compose files cho FastAPI stack.

        Args:
            config: InfrastructureConfig với backend_stack="fastapi"
