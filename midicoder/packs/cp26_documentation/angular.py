# coding: utf-8
"""
Mô-đun Angular emitter cho Documentation Generator (CP26).

Sinh code Storybook + TypeDoc cho Angular:
...
from midicoder.packs.cp26_documentation.models import DocCollection


class AngularDocEmitter:
    """Emitter sinh code Storybook + TypeDoc cho Angular."""
    ...
    def generate(
        ...
        """Generate toàn bộ doc files cho Angular."""
    ...
    def _generate_storybook_main(self, collection: DocCollection) -> list[dict[str, str]]:
        try:
            content = self.emitter.render("cp26_documentation/storybook_main.ts.jinja2", {"collection": collection})
    ...
    def _generate_storybook_preview(self, collection: DocCollection) -> list[dict[str, str]]:
        try:
            content = self.emitter.render("cp26_documentation/storybook_preview.ts.jinja2", {"collection": collection})
    ...
    def _generate_typedoc(self, collection: DocCollection) -> list[dict[str, str]]:
        try:
            content = self.emitter.render("cp26_documentation/typedoc.json.jinja2", {"collection": collection})
    ...
    def _generate_components(self, collection: DocCollection) -> list[dict[str, str]]:
        try:
            content = self.emitter.render("cp26_documentation/components.md.jinja2", {"collection": collection})
