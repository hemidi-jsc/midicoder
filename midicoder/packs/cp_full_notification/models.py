...
# ============================================================================
# Mobile Backend Models (from CP62)
# ============================================================================
...
class PushNotificationConfig:
    """Cấu hình push notification (FCM/APNs).
...
@dataclass
class DeepLinkRoute:
    """Route deep link với pattern matching.

    Attributes:
        id: ID duy nhất của deep link route
        path_pattern: Pattern của path (vd: "/product/{id}")
        target_screen: Tên screen/activity sẽ mở khi deep link được trigger
        auth_required: Có bắt buộc đăng nhập không
        params: Các tham số trong path pattern
        fallback_url: URL fallback khi deep link không được xử lý
        universal_link_enabled: Có bật Universal Link/App Link không
    """
    id: str
    path_pattern: str
    target_screen: str
...
class MobileAuthProvider:
    """OAuth2 mobile auth provider (Apple/Google/Facebook).
...
class OTAUpdateConfig:
    """Cấu hình OTA update với forced update và rollout.
...
class DeviceRegistration:
    """Đăng ký thiết bị mobile với push token.
    ...
    Attributes:
        ...
        deep_link_handler: Handler function cho deep link
