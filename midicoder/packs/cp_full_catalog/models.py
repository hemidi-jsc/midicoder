...
class Category:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.category_id or not self.category_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F32_CATEGORY_NOT_FOUND,
...
class ProductVariant:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.variant_id or not self.variant_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F32_VARIANT_NOT_FOUND,
        ...
        if not self.sku or not self.sku.strip():
            EM.raise_error(
                ErrorCode.MDC-F32_DUPLICATE_SKU,
...
class AttributeDefinition:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.attribute_id or not self.attribute_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F32_ATTRIBUTE_NOT_FOUND,
...
class CatalogEngine:
    ...
    def restore_product(self, product_id: str) -> Product:
        ...
        if not product.is_deleted:
            EM.raise_error(
                ErrorCode.MDC-F32_INVALID_PRODUCT_STATUS,
    ...
    def create_category(self, category: Category) -> Category:
        ...
        if category.parent_id is not None:
            ...
            if category.category_id in ancestor_ids:
                EM.raise_error(
                    ErrorCode.MDC-F32_CATEGORY_CYCLE_DETECTED,
            ...
            if depth > 3:
                EM.raise_error(
                    ErrorCode.MDC-F32_CATEGORY_DEPTH_EXCEEDED,
                    category_id=category.category_id,
                    current_depth=depth,
                    max_depth=3,
                )
