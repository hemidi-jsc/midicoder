...
class VersionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_versions < 0:
            EM.raise_error(ErrorCode.CP43_INVALID_VERSION_CONFIG, field="max_versions")
        if self.history_retention_days < 0:
            EM.raise_error(ErrorCode.CP43_INVALID_VERSION_CONFIG, field="history_retention_days")
...
class HistoryRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if self.version < 1:
            EM.raise_error(ErrorCode.CP43_INVALID_VERSION_NUMBER, version=self.version)
        if not self.entity_type or not self.entity_type.strip():
            EM.raise_error(ErrorCode.CP43_INVALID_VERSION_CONFIG, field="entity_type")
        if not self.entity_id or not self.entity_id.strip():
            EM.raise_error(ErrorCode.CP43_HISTORY_RECORD_NOT_FOUND, field="entity_id")
