...
class GeoPoint:
    ...
    def __post_init__(self) -> None:
        ...
        if not -90 <= self.latitude <= 90:
            EM.raise_error(ErrorCode.MDC-F23_INVALID_LATITUDE, value=self.latitude)
        if not -180 <= self.longitude <= 180:
            EM.raise_error(ErrorCode.MDC-F23_INVALID_LONGITUDE, value=self.longitude)
...
class Geofence:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(
                ErrorCode.MDC-F23_GEOSPEC_INVALID,
        ...
        if self.shape == GeofenceShape.CIRCLE:
            ...
            if self.radius_m <= 0:
                EM.raise_error(
                    ErrorCode.MDC-F23_GEOFENCE_EMPTY_RADIUS,
                    radius=self.radius_m,
                )
        elif self.shape == GeofenceShape.POLYGON:
            if len(self.points) < 3:
                EM.raise_error(
                    ErrorCode.MDC-F23_GEOFENCE_TOO_FEW_POINTS,
