...
class RouteDefinition:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.path or not self.path.strip():
            EM.raise_error(
                ErrorCode.MDC-F04_INVALID_ROUTE_PATH,
                field="path"
            )
        ...
        if not self.component or not self.component.strip():
            EM.raise_error(
                ErrorCode.MDC-F04_EMPTY_ROUTE_COMPONENT,
                field="component",
                path=self.path
            )
...
class StateStoreConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.store_type, StateStoreType):
            EM.raise_error(
                ErrorCode.MDC-F04_INVALID_STATE_STORE,
...
class FrontendApp:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.router_strategy, RouterStrategy):
            EM.raise_error(
                ErrorCode.MDC-F04_INVALID_ROUTER_STRATEGY,
        ...
        for route in self.routes:
            if isinstance(route, RouteDefinition):
                if route.path in seen_paths:
                    EM.raise_error(
                        ErrorCode.MDC-F04_DUPLICATE_ROUTE,
                        path=route.path
                    )
