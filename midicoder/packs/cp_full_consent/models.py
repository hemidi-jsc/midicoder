...
class CookiePreference:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.user_id or not self.user_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F31_COOKIE_CATEGORY_INVALID,
...
class CommunicationPreference:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.user_id or not self.user_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F31_COMM_CHANNEL_INVALID,
    ...
    def revoke_consent(self, record_id: str, reason: str = "") -> ConsentRecord:
        ...
        for policy in self.policies.values():
            if (policy.tenant_id == record.tenant_id
                ...
                EM.raise_error(
                    ErrorCode.MDC-F31_MANDATORY_CONSENT_CANNOT_REVOKE,
                    record_id=record_id,
