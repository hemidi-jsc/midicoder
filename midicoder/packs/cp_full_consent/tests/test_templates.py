...
# __file__ = .../midicoder/packs/cp_full_consent/tests/test_templates.py
# parent x6 = midicoder-ce/
PROJECT_ROOT = Path(__file__).parent.parent.parent.parent.parent.parent
EMITTERS_DIR = PROJECT_ROOT / "midicoder" / "emitters" / "core" / "cp_full_consent"
...
class TestPackManifest:
    ...
    def test_pack_internal_id(self):
        ...
        with open(EMITTERS_DIR / "pack.yml", "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        assert data["pack"]["internal_id"] == "cp_full_consent"
...
class TestFastAPITemplates:
    ...
    def test_all_templates_exist(self):
        ...
        stack_dir = STACKS_DIR / "fastapi" / "core" / "cp_full_consent"
...
class TestNestJSTemplates:
    ...
    def test_all_templates_exist(self):
        ...
        stack_dir = STACKS_DIR / "nestjs" / "core" / "cp_full_consent"
        for template in self.expected_templates:
            assert (stack_dir / template).exists(), f"Thiếu template: {template}"
...
class TestAngularTemplates:
    ...
    def test_all_templates_exist(self):
        ...
        stack_dir = STACKS_DIR / "angular" / "core" / "cp_full_consent"
        for template in self.expected_templates:
            assert (stack_dir / template).exists(), f"Thiếu template: {template}"
...
class TestReactTemplates:
    ...
    def test_all_templates_exist(self):
        ...
        stack_dir = STACKS_DIR / "react" / "core" / "cp_full_consent"
        for template in self.expected_templates:
            assert (stack_dir / template).exists(), f"Thiếu template: {template}"
...
class TestRegistry:
    ...
    def test_cp49_in_registry(self):
        ...
        assert CP_ID_TO_INTERNAL["CP49"] == "cp_full_consent"
...
class TestInitModule:
    ...
    def test_init_exports_models(self):
        ...
        from midicoder.packs.cp_full_consent import (
            ConsentRecord,
            ConsentPolicy,
            CookiePreference,
...
class TestTemplateRender:
    ...
    def _render_all_fastapi(self):
        ...
        from midicoder.packs.cp_full_consent.fastapi import FastAPIConsentEmitter
        from midicoder.packs.cp_full_consent.recipes import basic_consent_recipe
        emitter = FastAPIConsentEmitter(stack_dir=str(STACKS_DIR / "fastapi" / "core" / "cp_full_consent"))
    ...
    def _render_all_nestjs(self):
        ...
        from midicoder.packs.cp_full_consent.nestjs import NestJSConsentEmitter
        from midicoder.packs.cp_full_consent.recipes import basic_consent_recipe
        emitter = NestJSConsentEmitter(stack_dir=str(STACKS_DIR / "nestjs" / "core" / "cp_full_consent"))
        ir = basic_consent_recipe().ir
        files = emitter.emit(ir, "/tmp")
        return [(f.path, f.content) for f in files]
    ...
    def _render_all_angular(self):
        ...
        from midicoder.packs.cp_full_consent.angular import AngularConsentEmitter
        from midicoder.packs.cp_full_consent.recipes import basic_consent_recipe
        emitter = AngularConsentEmitter(stack_dir=str(STACKS_DIR / "angular" / "core" / "cp_full_consent"))
        ir = basic_consent_recipe().ir
        files = emitter.emit(ir, "/tmp")
        return [(f.path, f.content) for f in files]
    ...
    def _render_all_react(self):
        ...
        from midicoder.packs.cp_full_consent.react import ReactConsentEmitter
        from midicoder.packs.cp_full_consent.recipes import basic_consent_recipe
        emitter = ReactConsentEmitter(stack_dir=str(STACKS_DIR / "react" / "core" / "cp_full_consent"))
        ir = basic_consent_recipe().ir
