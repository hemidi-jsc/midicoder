...
class TenantRegistration:
    ...
    def __post_init__(self) -> None:
        ...
        if not re.match(email_pattern, self.email.strip()):
            EM.raise_error(
                ErrorCode.MDC-F14_INVALID_EMAIL,
                email=self.email,
        ...
        if not self.company_name or not self.company_name.strip():
            EM.raise_error(
                ErrorCode.MDC-F14_EMPTY_COMPANY_NAME,
        ...
        if self.trial_days < 0:
            EM.raise_error(
                ErrorCode.MDC-F14_INVALID_TRIAL_DAYS,
...
class TenantSubscription:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.tenant_id or not self.tenant_id.strip():
            EM.raise_error(
                ErrorCode.MDC-F14_SUBSCRIPTION_NOT_FOUND,
        ...
        if self.price < Decimal("0"):
            EM.raise_error(
                ErrorCode.MDC-F14_INVALID_PLAN,
