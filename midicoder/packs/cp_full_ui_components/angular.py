...
Templates: stacks/angular/cp_full_ui_components/**/*.component.ts.jinja2
...
_TEMPLATE_DIR = _PACKAGE_DIR / "stacks" / "angular" / "cp_full_ui_components"
...
class AngularUIEmitter:
    ...
    def _write_file(
        ...
        return GeneratedFile(
            path=Path(filename),
            content=content,
            template=f"angular/cp_full_ui_components/{template_name}",
        )
...
def rc_component(render_context, component_name):
    ...
    if hasattr(render_context, "get_component"):
        ...
        for attr in ["sortable", "paginated", "page_size", "selectable",
            ...
            if v is not None:
                b[attr] = v
        tokens = override.tokens.get_for_framework("material")
