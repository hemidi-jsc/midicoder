# coding: utf-8
"""
Mô-đun models cho I04 — Environment & Secret Management.
...
KPI: CP Obligations Coverage (>= 2 obligations cho I04).
...
class SecretConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            raise EM.raise_error(
                ErrorCode.MDC-I05_SECRET_CONFIG_INVALID,
                reason="id bắt buộc và không được để trống",
            )
...
class VaultConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            raise EM.raise_error(
                ErrorCode.MDC-I05_VAULT_CONFIG_INVALID,
                reason="id bắt buộc và không được để trống",
            )
...
class KMSConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            raise EM.raise_error(
                ErrorCode.MDC-I05_KMS_CONFIG_INVALID,
                reason="id bắt buộc và không được để trống",
            )
...
class ConfigMapRef:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            raise EM.raise_error(
                ErrorCode.MDC-I05_CONFIGMAP_REF_INVALID,
                reason="id bắt buộc và không được để trống",
            )
