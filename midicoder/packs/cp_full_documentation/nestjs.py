...
class NestJSDocEmitter:
    ...
    def _generate_swagger(self, collection: DocCollection) -> list[dict[str, str]]:
        try:
            content = self.emitter.render("cp_full_documentation/swagger_config.ts.jinja2", {"collection": collection})
    ...
    def _generate_docusaurus(self, collection: DocCollection) -> list[dict[str, str]]:
        try:
            content = self.emitter.render("cp_full_documentation/docusaurus.config.js.jinja2", {"collection": collection})
    ...
    def _generate_sidebar(self, collection: DocCollection) -> list[dict[str, str]]:
        try:
            content = self.emitter.render("cp_full_documentation/sidebar.js.jinja2", {"collection": collection})
    ...
    def _generate_index(self, collection: DocCollection) -> list[dict[str, str]]:
        try:
            content = self.emitter.render("cp_full_documentation/docs_index.md.jinja2", {"collection": collection})
