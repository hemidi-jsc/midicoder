...
class ApiDocConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.openapi_version not in valid_versions:
            EM.raise_error(
                ErrorCode.MDC-F09_INVALID_API_VERSION,
                version=self.openapi_version,
                valid=list(valid_versions)
            )
        if not self.title or not self.title.strip():
            EM.raise_error(ErrorCode.MDC-F09_MISSING_SECTION_TITLE, field="api_doc.title")
...
class DocSection:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.title or not self.title.strip():
            EM.raise_error(
                ErrorCode.MDC-F09_MISSING_SECTION_TITLE,
                section_id=self.id
            )
        if not self.path or not self.path.strip():
            EM.raise_error(
                ErrorCode.MDC-F09_EMPTY_SECTION_PATH,
                section_id=self.id
            )
        valid_sources = {"entities", "commands", "queries", "events", "manual"}
        if self.content_source not in valid_sources:
            EM.raise_error(
                ErrorCode.MDC-F09_SECTION_SOURCE_INVALID,
...
class DocPortal:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.title or not self.title.strip():
            EM.raise_error(ErrorCode.MDC-F09_EMPTY_PORTAL_NAME, field="portal.title")

    def add_section(self, section: DocSection) -> None:
        """Thêm section vào portal."""
        if self.get_section_by_id(section.id):
            EM.raise_error(
                ErrorCode.MDC-F09_DUPLICATE_SECTION_ID,
                id=section.id,
                portal_id=self.id
            )
