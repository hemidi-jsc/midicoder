...
class FastAPIDocEmitter:
    ...
    def _generate_mkdocs(self, collection: DocCollection) -> list[dict[str, str]]:
        ...
        try:
            content = self.emitter.render("cp_full_documentation/mkdocs.yml.jinja2", {"collection": collection})
    ...
    def _generate_openapi(
        ...
        """Sinh OpenAPI spec."""
        try:
            content = self.emitter.render("cp_full_documentation/openapi.json.jinja2", {
    ...
    def _generate_nav(self, collection: DocCollection) -> list[dict[str, str]]:
        """Sinh nav.md."""
        try:
            content = self.emitter.render("cp_full_documentation/nav.md.jinja2", {"collection": collection})
