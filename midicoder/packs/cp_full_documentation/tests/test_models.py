...
class TestApiDocConfig:
    ...
    def test_invalid_version(self) -> None:
        ...
        assert exc_info.value.code == ErrorCode.MDC-F09_INVALID_API_VERSION

    def test_empty_title(self) -> None:
        """Title rỗng throw error."""
        with pytest.raises(MidicoderError) as exc_info:
            ApiDocConfig(id="x", title="")
        assert exc_info.value.code == ErrorCode.MDC-F09_MISSING_SECTION_TITLE
...
class TestDocSection:
    ...
    def test_empty_path(self) -> None:
        ...
        assert exc_info.value.code == ErrorCode.MDC-F09_EMPTY_SECTION_PATH

    def test_invalid_source(self) -> None:
        """Source không hợp lệ throw error."""
        with pytest.raises(MidicoderError) as exc_info:
            DocSection(id="s1", portal_id="p1", title="T", path="x.md", content_source="invalid")
        assert exc_info.value.code == ErrorCode.MDC-F09_SECTION_SOURCE_INVALID
...
class TestDocPortal:
    ...
    def test_empty_title(self) -> None:
        ...
        assert exc_info.value.code == ErrorCode.MDC-F09_EMPTY_PORTAL_NAME
    ...
    def test_duplicate_section(self) -> None:
        ...
        assert exc_info.value.code == ErrorCode.MDC-F09_DUPLICATE_SECTION_ID
