...
class TranslationEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.key or not self.key.strip():
            EM.raise_error(ErrorCode.MDC-F16_EMPTY_TRANSLATION_KEY)

        # Kiểm tra namespace hợp lệ (snake_case)
        if self.namespace and not re.match(r'^[a-z][a-z0-9_]*$', self.namespace):
            EM.raise_error(ErrorCode.MDC-F16_INVALID_NAMESPACE, namespace=self.namespace)
...
class CacheConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.ttl_seconds <= 0:
            EM.raise_error(
                ErrorCode.MDC-F16_CACHE_CONFIG_INVALID,
...
class LocaleFormatter:
    ...
    def format_date(self, dt: datetime, locale_code: str) -> str:
        ...
        if not locale:
            EM.raise_error(ErrorCode.MDC-F16_FORMATTER_INVALID_LOCALE, locale_code=locale_code)
...
# ===========================================================================
# CP29 — Key Extraction Models (merged from cp29_multi_language)
# ===========================================================================
...
class I18nKey:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.key or not self.key.strip():
            EM.raise_error(ErrorCode.MDC-F16_EMPTY_KEY)

        if self.namespace not in _VALID_NAMESPACES:
            EM.raise_error(
                ErrorCode.MDC-F16_INVALID_KEY_FORMAT,
                namespace=self.namespace,
...
class LanguageProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-F16_EMPTY_PROFILE_ID)

        if not self.locales:
            EM.raise_error(
                ErrorCode.MDC-F16_INVALID_LOCALE,
        ...
        if len(self.locales) != len(set(self.locales)):
            EM.raise_error(
                ErrorCode.MDC-F16_DUPLICATE_LOCALE,
...
class I18nKeyset:
    ...
    def add_key(self, key: I18nKey) -> None:
        ...
        if any(existing.key == key.key for existing in self.keys):
            EM.raise_error(ErrorCode.MDC-F16_KEY_CONFLICT, key=key.key)
