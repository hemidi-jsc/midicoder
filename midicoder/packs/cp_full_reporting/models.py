...
class ReportSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-F22_EMPTY_REPORT_ID)
        # Entity không được để trống
        if not self.entity or not self.entity.strip():
            EM.raise_error(ErrorCode.MDC-F22_REPORT_SPEC_INVALID, reason="entity không được để trống")
...
class BatchJob:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.report_spec_ids:
            EM.raise_error(
                ErrorCode.MDC-F22_BATCH_JOB_FAILED,
