...
"""
        ...
        assert exc_info.value.code == ErrorCode.MDC-F22_INVALID_FORMAT
...
"""
        with pytest.raises(MidicoderError) as exc_info:
            self.parser.parse(dsl)
        assert exc_info.value.code == ErrorCode.MDC-F22_INVALID_LAYOUT
...
"""
    ...
    def test_parse_invalid_yaml_raises_error(self):
        ...
        with pytest.raises(MidicoderError) as exc_info:
            self.parser.parse(dsl)
        assert exc_info.value.code == ErrorCode.MDC-F22_PARSER_ERROR
