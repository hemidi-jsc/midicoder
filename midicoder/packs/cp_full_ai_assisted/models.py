...
class ReviewPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-F18_EMPTY_REVIEW_POLICY_ID, field="policy.id")
        if not self.severity:  # pragma: no cover
            EM.raise_error(ErrorCode.MDC-F18_INVALID_SEVERITY, field="policy.severity")
        if not self.category:  # pragma: no cover
            EM.raise_error(ErrorCode.MDC-F18_INVALID_CATEGORY, field="policy.category")
...
class ReviewResult:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.finding_id or not self.finding_id.strip():
            EM.raise_error(ErrorCode.MDC-F18_EMPTY_REVIEW_RESULT_ID, field="result.finding_id")
...
class SuggestionResult:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-F18_EMPTY_SUGGESTION_RESULT_ID, field="result.id")
        if not self.suggestion_type:  # pragma: no cover
            EM.raise_error(ErrorCode.MDC-F18_INVALID_SUGGESTION_TYPE, field="result.suggestion_type")
...
class PromptTemplate:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-F18_EMPTY_TEMPLATE_ID, field="template.id")
        if not self.category:  # pragma: no cover
            EM.raise_error(ErrorCode.MDC-F18_INVALID_PROMPT_CATEGORY, field="template.category")
...
class AIAssistantConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-F18_EMPTY_CONFIG_ID, field="config.id")
...
class AIAssistantCollection:
    ...
    def add_review_policy(self, policy: ReviewPolicy) -> None:
        ...
        if self.get_review_policy_by_id(policy.id):
            EM.raise_error(
                ErrorCode.MDC-F18_DUPLICATE_REVIEW_POLICY_ID,
                id=policy.id,
    ...
    def add_prompt_template(self, template: PromptTemplate) -> None:
        ...
        if self.get_template_by_id(template.id):
            EM.raise_error(
                ErrorCode.MDC-F18_DUPLICATE_TEMPLATE_ID,
                id=template.id,
