# coding: utf-8
"""
Mô-đun React emitter cho Performance Testing Generator (CP25).

Sinh code Lighthouse CI + Playwright cho React:
- lighthouserc.js — Lighthouse config
- src/__tests__/perf/web-vitals.test.tsx — Playwright Web Vitals
...
class ReactPerfEmitter:
    """
    Emitter sinh code Lighthouse CI + Playwright cho React.
    ...
    def generate(
        ...
        """Generate toàn bộ perf test files cho React."""
    ...
    def _generate_web_vitals(self, collection: PerfCollection) -> list[dict[str, str]]:
        ...
        try:
            content = self.emitter.render(
                "cp25_performance_testing/web_vitals.test.tsx.jinja2",
                {"collection": collection},
            )
            return [{"path": "src/__tests__/perf/web-vitals.test.tsx", "content": content}]
        except Exception:
            return []
