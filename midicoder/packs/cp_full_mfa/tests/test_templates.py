...
class TestPackManifest:
    ...
    def test_pack_internal_id(self):
        with open(PACK_DIR / "pack.yml", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        assert data["pack"]["internal_id"] == "cp_full_mfa"
...
class TestFastAPITemplates:
    ...
    def template_dir(self):
        ...
        return root / "stacks" / "fastapi" / "core" / "cp_full_mfa"
...
class TestNestJSTemplates:
    ...
    def template_dir(self):
        root = Path(__file__).resolve().parent.parent.parent.parent.parent
        return root / "stacks" / "nestjs" / "core" / "cp_full_mfa"
...
class TestAngularTemplates:
    ...
    def template_dir(self):
        root = Path(__file__).resolve().parent.parent.parent.parent.parent
        return root / "stacks" / "angular" / "core" / "cp_full_mfa"
...
class TestReactTemplates:
    ...
    def template_dir(self):
        root = Path(__file__).resolve().parent.parent.parent.parent.parent
        return root / "stacks" / "react" / "core" / "cp_full_mfa"
...
class TestRegistry:
    def test_cp46_in_registry(self):
        ...
        assert reg.CP_ID_TO_INTERNAL["CP46"] == "cp_full_mfa"
...
class TestInitModule:
    ...
    def test_init_exports_models(self):
        from midicoder.packs.cp_full_mfa import (
            MFAMethod,
            MFAMethodStatus,
            MFAPriority,
            MFAChallenge,
...
def _render_template(stack: str, template_name: str) -> str:
    ...
    """
    template_dir = STACKS_DIR / stack / "core" / "cp_full_mfa"
