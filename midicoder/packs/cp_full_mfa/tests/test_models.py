...
class TestCP46ErrorCodes:
    def test_mfa_factor_not_found_code(self):
        assert ErrorCode.MDC-F27_MFA_FACTOR_NOT_FOUND.value == "MDC-CP46-001"

    def test_mfa_already_enabled_code(self):
        assert ErrorCode.MDC-F27_MFA_ALREADY_ENABLED.value == "MDC-CP46-002"

    def test_mfa_not_enabled_code(self):
        assert ErrorCode.MDC-F27_MFA_NOT_ENABLED.value == "MDC-CP46-003"

    def test_invalid_otp_code(self):
        assert ErrorCode.MDC-F27_INVALID_OTP_CODE.value == "MDC-CP46-004"

    def test_otp_expired_code(self):
        assert ErrorCode.MDC-F27_OTP_EXPIRED.value == "MDC-CP46-005"

    def test_totp_secret_invalid_code(self):
        assert ErrorCode.MDC-F27_TOTP_SECRET_INVALID.value == "MDC-CP46-006"

    def test_webauthn_registration_failed_code(self):
        assert ErrorCode.MDC-F27_WEBAUTHN_REGISTRATION_FAILED.value == "MDC-CP46-007"

    def test_webauthn_verification_failed_code(self):
        assert ErrorCode.MDC-F27_WEBAUTHN_VERIFICATION_FAILED.value == "MDC-CP46-008"

    def test_biometric_not_supported_code(self):
        assert ErrorCode.MDC-F27_BIOMETRIC_NOT_SUPPORTED.value == "MDC-CP46-009"

    def test_mfa_rate_limit_exceeded_code(self):
        assert ErrorCode.MDC-F27_MFA_RATE_LIMIT_EXCEEDED.value == "MDC-CP46-010"
