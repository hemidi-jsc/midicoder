...
# __file__ = .../midicoder/packs/cp_full_bulk_etl/tests/test_templates.py
# parent x6 = midicoder-ce/
PROJECT_ROOT = Path(__file__).parent.parent.parent.parent.parent.parent
EMITTERS_DIR = PROJECT_ROOT / "midicoder" / "emitters" / "core" / "cp_full_bulk_etl"
...
class TestPackManifest:
    ...
    def test_pack_internal_id(self):
        ...
        with open(EMITTERS_DIR / "pack.yml", "r", encoding="utf-8") as f:
            data = yaml.safe_load(f)
        assert data["pack"]["internal_id"] == "cp_full_bulk_etl"
...
class TestFastAPITemplates:
    ...
    def test_all_templates_exist(self):
        ...
        stack_dir = STACKS_DIR / "fastapi" / "core" / "cp_full_bulk_etl"
...
class TestNestJSTemplates:
    ...
    def test_all_templates_exist(self):
        ...
        stack_dir = STACKS_DIR / "nestjs" / "core" / "cp_full_bulk_etl"
        for template in self.expected_templates:
            assert (stack_dir / template).exists(), f"Thiếu template: {template}"
...
class TestAngularTemplates:
    ...
    def test_all_templates_exist(self):
        ...
        stack_dir = STACKS_DIR / "angular" / "core" / "cp_full_bulk_etl"
        for template in self.expected_templates:
            assert (stack_dir / template).exists(), f"Thiếu template: {template}"
...
class TestReactTemplates:
    ...
    def test_all_templates_exist(self):
        ...
        stack_dir = STACKS_DIR / "react" / "core" / "cp_full_bulk_etl"
        for template in self.expected_templates:
            assert (stack_dir / template).exists(), f"Thiếu template: {template}"
...
class TestRegistry:
    ...
    def test_cp44_in_registry(self):
        ...
        assert CP_ID_TO_INTERNAL["CP44"] == "cp_full_bulk_etl"
...
class TestInitModule:
    ...
    def test_init_exports_models(self):
        ...
        from midicoder.packs.cp_full_bulk_etl import (
            BulkAction,
            BulkJob,
            BulkEngine,
            JobStatus,
...
def _render_template(stack: str, template_name: str) -> str:
    ...
    """
    template_dir = STACKS_DIR / stack / "core" / "cp_full_bulk_etl"
