...
class ReplicaConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="replica.name")
        # Connection string không được để trống
        if not self.connection_string or not self.connection_string.strip():
            EM.raise_error(ErrorCode.MDC-BE01_MISSING_DATASOURCE, field="replica.connection_string", replica=self.name)
        # Weight phải dương
        if self.weight < 1:
            self.weight = 1
...
class Datasource:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="datasource.name")
        # Connection string không được để trống
        if not self.connection_string or not self.connection_string.strip():
            EM.raise_error(ErrorCode.MDC-BE01_MISSING_DATASOURCE, datasource=self.name)
...
class ColumnDef:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="column.name")
...
class Relationship:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.target_model or not self.target_model.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="relationship.target_model")
...
class IndexDef:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="index.name")
...
class DataModel:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="model.id")
        # Table name không được để trống
        if not self.table_name or not self.table_name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="model.table_name", model=self.id)
        # Kiểm tra duplicate column names
        if self._has_duplicate_columns():
            EM.raise_error(ErrorCode.MDC-BE01_DUPLICATE_COLUMN_NAME, model=self.id)
        # Kiểm tra missing primary key (nếu có columns)
        if self.columns and not self.get_primary_key():
            EM.raise_error(ErrorCode.MDC-BE01_MISSING_PRIMARY_KEY, model=self.id, table=self.table_name)
...
class DataModelCollection:
    ...
    def add_model(self, model: DataModel) -> None:
        ...
        if self.get_by_table_name(model.table_name):
            EM.raise_error(ErrorCode.MDC-BE01_DUPLICATE_TABLE_NAME, table=model.table_name)
...
class SpatialColumn:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="spatial_column.name")
        if self.srid < 1:
            self.srid = 4326
...
class SpatialIndex:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="spatial_index.name")
        if not self.column or not self.column.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="spatial_index.column")
...
class RetentionPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="retention_policy.name")
        if self.threshold < 0:
            self.threshold = 0
...
class TimeSeriesModel:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="time_series.id")
        if not self.table_name or not self.table_name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="time_series.table_name")
        if not self.timestamp_column or not self.timestamp_column.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="time_series.timestamp_column")
...
class ReadModel:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="read_model.id")
        if not self.table_name or not self.table_name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="read_model.table_name")
...
class TransactionParticipant:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.datasource_name or not self.datasource_name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="transaction_participant.datasource_name")
...
class DistributedTransaction:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id or not self.id.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="distributed_tx.id")
        if not self.coordinator_datasource or not self.coordinator_datasource.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="distributed_tx.coordinator_datasource")
        if self.timeout_seconds < 1:
...
class JSONBIndex:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="jsonb_index.name")
        if not self.column or not self.column.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="jsonb_index.column")
...
class JSONBColumn:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(ErrorCode.MDC-BE01_EMPTY_NAME, field="jsonb_column.name")
