...
class StorageProfile:
    ...
    def __post_init__(self):
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(
                ErrorCode.MDC-F35_EMPTY_PROFILE_NAME,
        ...
        if self.backend_type not in StorageBackend:
            EM.raise_error(
                ErrorCode.MDC-F35_INVALID_BACKEND_TYPE,
        ...
        if self.backend_type == StorageBackend.S3:
            if not self.bucket or not self.bucket.strip():
                EM.raise_error(
                    ErrorCode.MDC-F35_EMPTY_BUCKET_NAME,
                    profile_name=self.name,
                )
...
class UploadPolicy:
    ...
    def __post_init__(self):
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(
                ErrorCode.MDC-F35_MISSING_POLICY,
        ...
        if not self.allowed_content_types:
            EM.raise_error(
                ErrorCode.MDC-F35_INVALID_CONTENT_TYPE,
        ...
        if self.max_file_size <= 0:
            EM.raise_error(
                ErrorCode.MDC-F35_FILE_SIZE_EXCEEDED,
                policy_name=self.name,
        ...
        if not self.allowed_extensions:
            EM.raise_error(
                ErrorCode.MDC-F35_INVALID_EXTENSION,
                policy_name=self.name,
...
class MediaTransform:
    ...
    def __post_init__(self):
        ...
        if not self.name or not self.name.strip():
            EM.raise_error(
                ErrorCode.MDC-F35_TRANSFORM_INVALID_PARAM,
