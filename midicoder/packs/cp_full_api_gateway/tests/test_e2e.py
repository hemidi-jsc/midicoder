...
from midicoder.packs.cp_full_api_gateway.fastapi import FastAPIGatewayEmitter
from midicoder.packs.cp_full_api_gateway.nestjs import NestJSGatewayEmitter
from midicoder.packs.cp_full_api_gateway.angular import AngularGatewayEmitter
from midicoder.packs.cp_full_api_gateway.react import ReactGatewayEmitter
...
def run_e2e():
    ...
    try:
        ...
        assert CP_ID_TO_INTERNAL["CP06"] == "cp_full_api_gateway", f"Wrong mapping: {CP_ID_TO_INTERNAL['CP06']}"
        results.append(("Pack registry", True, "CP06 → cp_full_api_gateway registered"))
        passed += 1
    ...
    try:
        ...
        pack = data["pack"]
        assert pack["id"] == "CP06"
        assert pack["internal_id"] == "cp_full_api_gateway"
    ...
    try:
        ...
        fc = loader.load(pack_internal_id="cp_full_api_gateway", pack_id="CP06", stack="fastapi")
    ...
    try:
        ...
        try:
            template_dir = Path(tmpdir) / "core" / "cp_full_api_gateway"
            ...
            react_template_dir = Path(tmpdir) / "core" / "cp_full_api_gateway"
    ...
    try:
        ...
        required_templates = {
            "fastapi": [
                "cp_full_api_gateway/__init__.py.jinja2",
                "cp_full_api_gateway/circuit_breaker.py.jinja2",
                "cp_full_api_gateway/gateway_app.py.jinja2",
                "cp_full_api_gateway/gateway_router.py.jinja2",
                "cp_full_api_gateway/gateway_service.py.jinja2",
                "cp_full_api_gateway/rate_limiter.py.jinja2",
                "cp_full_api_gateway/route_config.py.jinja2",
                "cp_full_api_gateway/http_route.py.jinja2",
            ],
            "nestjs": [
                "cp_full_api_gateway/gateway.module.ts.jinja2",
                "cp_full_api_gateway/gateway.service.ts.jinja2",
                "cp_full_api_gateway/gateway.controller.ts.jinja2",
                "cp_full_api_gateway/circuit-breaker.providers.ts.jinja2",
                "cp_full_api_gateway/rate-limiter.providers.ts.jinja2",
                "cp_full_api_gateway/route-config.service.ts.jinja2",
                "cp_full_api_gateway/http_route.ts.jinja2",
            ],
            "angular": [
                "cp_full_api_gateway/gateway.module.ts.jinja2",
                "cp_full_api_gateway/api-client.service.ts.jinja2",
                "cp_full_api_gateway/route-guard.service.ts.jinja2",
            ],
            "react": [
                "cp_full_api_gateway/api-client.ts.jinja2",
                "cp_full_api_gateway/route-guard.tsx.jinja2",
                "cp_full_api_gateway/types.ts.jinja2",
            ],
        }
        missing = []
