    ...
    def __post_init__(self) -> None:
        ...
        if not self.code or len(self.code) != 3 or not self.code.isalpha():
            raise EM.raise_error(
                ErrorCode.MDC-F21_CURRENCY_INVALID,
                currency=self.code,
        ...
        if not 0 <= self.decimal_places <= 6:
            raise EM.raise_error(
                ErrorCode.MDC-F21_INVALID_DECIMAL_PRECISION,
                currency=self.code,
...
class FXRate:
    ...
    def __post_init__(self) -> None:
        ...
        if self.from_currency == self.to_currency:
            raise EM.raise_error(
                ErrorCode.MDC-F21_INVALID_CURRENCY_PAIR,
        ...
        try:
            ...
            raise EM.raise_error(
                ErrorCode.MDC-F21_FX_RATE_NOT_FOUND,
...
class Account:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.code:
            raise EM.raise_error(
                ErrorCode.MDC-F21_ACCOUNT_NOT_FOUND,
...
class LedgerEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id:
            raise EM.raise_error(
                ErrorCode.MDC-F21_INVALID_ENTRY_TYPE,
...
class FinancialTransaction:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id:
            raise EM.raise_error(
                ErrorCode.MDC-F21_TRANSACTION_NOT_FOUND,
    ...
    def validate_balance(self) -> bool:
        ...
        if self.transaction_type == TransactionType.DOUBLE_ENTRY:
            if self.total_debit != self.total_credit:
                raise EM.raise_error(
                    ErrorCode.MDC-F21_DOUBLE_ENTRY_IMBALANCE,
...
class RoundingRule:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.currency_code or len(self.currency_code) != 3:
            raise EM.raise_error(
                ErrorCode.MDC-F21_ROUNDING_RULE_NOT_FOUND,
...
class LedgerSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.id:
            raise EM.raise_error(
                ErrorCode.MDC-F21_HASH_CHAIN_FAILED,
