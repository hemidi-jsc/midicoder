...
FASTAPI_DIR = MIDICODER_ROOT / "midicoder" / "stacks" / "fastapi" / "core" / "cp_full_approval"
NESTJS_DIR = MIDICODER_ROOT / "midicoder" / "stacks" / "nestjs" / "core" / "cp_full_approval"
ANGULAR_DIR = MIDICODER_ROOT / "midicoder" / "stacks" / "angular" / "core" / "cp_full_approval"
REACT_DIR = MIDICODER_ROOT / "midicoder" / "stacks" / "react" / "core" / "cp_full_approval"
PACK_DIR = MIDICODER_ROOT / "midicoder" / "emitters" / "core" / "cp_full_approval"
...
class TestPackYaml:
    ...
    def test_pack_yml_has_cp42_id(self):
        """Kiểm tra pack.yml chứa định danh CP42 và cp_full_approval."""
        pack_yml = PACK_DIR / "pack.yml"
        content = pack_yml.read_text(encoding="utf-8")
        assert "CP42" in content
        assert "cp_full_approval" in content
...
class TestRegistry:
    ...
    def test_cp42_in_registry(self):
        """Kiểm tra CP42 ánh xạ đúng với cp_full_approval trong registry."""
        from midicoder.contracts.registry import CP_ID_TO_INTERNAL

        assert "CP42" in CP_ID_TO_INTERNAL
        assert CP_ID_TO_INTERNAL["CP42"] == "cp_full_approval"
