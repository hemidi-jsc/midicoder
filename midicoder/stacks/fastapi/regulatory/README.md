# Regulatory Overlay Templates (FastAPI Core)

## Purpose

This directory holds **regulatory/compliance overlay templates** that can be applied on top of the FastAPI core capability-pack templates during code generation. Each overlay adds the files and configuration required to meet specific industry or jurisdictional compliance frameworks.
...
2. If a regulatory framework is specified in the generation configuration (e.g. `--regulatory=hipaa`), the generator loads the corresponding subdirectory here.
...
- Example: to override `cp14_audit_compliance/audit_log.py.jinja2`, place the override at `regulatory/hipaa/cp14_audit_compliance/audit_log.py.jinja2`.
